...
\subsection{Topologie forte et topologie faible sur les espaces de Banach}
...
\((E', \left\Vert \cdot \right\Vert')\) est un espace de Banach, un cas particulier de \(\mathscr{L}(E,F)\), avecn pour \(u \in \mathscr{L}(E,F)\),

\[\left\Vert u \right\Vert _{\mathscr{L}(E,F)} = \sup_{ x \in E, \left\Vert x \right\Vert_E \leq 1 }  \left\Vert u(x) \right\Vert_F. \]
...
\subsubsection{Comparaison des topologies \((E, \left\Vert \cdot \right\Vert)\) et \(\sigma(E,E')\)}

\begin{lemma}
  Soit \(E\) un espace de Banach. Alors la norme définie

  \[x = \sup_{ l \in E', \left\Vert l \right\Vert \leq 1  } \left\lvert \langle l,x \rangle  \right\rvert = \langle l_0,x \rangle.  \]
...
\begin{proof}[Autre démonstration]
  ...
  Or \(\left\lvert \langle l_i,x \rangle  \right\rvert \less \varepsilon\) ...
\end{proof}
...
\begin{proof}
  ...
  Par Hahn-Banach, pour \(A = \{ x_1 \}, B = \{ x_2 \} \) compacts et convexes. Pour tout \((x,y) \in A \times B\), on a
...
\begin{proof}
  ...
  On a \(g(0) = \left\Vert x_0 \right\Vert \less 1\) et \(g(\infty) = +\infty\). \(g\) est continue, donc il existe \(t_0 \in (0, \infty)\) tel que \(\left\Vert x_0 + t_0 y_0 \right\Vert = 1\). Donc \(x_0 + t_0 y_0 \in S\) et \(x_0 + t_0 y_0 \in V\), car
...
\begin{remark}
  ...
  Si \(E\) est réfléxif, alors \(B_E = \{ \left\Vert x \right\Vert \leq 1 \} \) est un espace métrique compact pour \(\mathcal{O}(E, E')\).
\end{remark}

\subsection{Théorème de Banach-Steinhaus, suites faiblement et fortement convergentes}

\begin{thm}
  Soient \(E,F\) espaces de Banach et \((T_a) _{a \in A} \in \mathscr{L}(E,F)\) tel que
  ...
  ie \(\exists C \bg 0, \forall x \in E, \forall \alpha \in A, \left\Vert T_\alpha(x)  \leq C \left\Vert x \right\Vert  \right\Vert\).
\end{thm}
...
\paragraph{Suites faiblement et fortement convergentes}

\begin{definition}[Suite fortement convergente]
  \(x_n \in E, x_n \longrightarrow x \iff \left\Vert x_n -x \right\Vert \longrightarrow 0\).
\end{definition}

\begin{definition}[Suite fortement convergente]
  \(x_n \in E, x_n \longrightarrow x \iff \forall l \in E', \langle l,x_n \rangle \longrightarrow \langle l,x \rangle\).
\end{definition}
