...
\begin{definition}
  Deux représentations linéaires \(\rho : G \to GL(V)\) et \(\rho' : G \to GL(V')\) d'un groupe \(G\) sont dites \textbf{isomorphes} ou équivalentes s'il existe un isomorphisme d'espaces vectoriels \(f: V \to V'\) tel que l'on ait : \[\forall g \in G, \rho'_g \circ f  = f \circ \rho_g.\]
...
L'application \(p : \begin{matrix}
  V & \longrightarrow & V \\
  v = \stackrel{\in W}{w} + \stackrel{\in W'}{w'} & \longmapsto & w
...
\begin{lemma}
  Tout sous-espace stable d'une représentation linéaire complexe de degré fini d'un groupe fini admet un sous-espace \textbf{supplémentaire \textit{stable}}.
\end{lemma}
...
\begin{prop}
  ...
  \begin{enumerate}
    ...
    \item \(\forall g, h \in G\), \(\chi _{\rho}(g h g ^{-1} ) = \chi _{\rho}(h)\), i. e. \(\chi _{\rho}\) est une fonction centrale sur \(G\), i. e. \(\chi _{\rho}\) est constante sur les classes de conjugaison.
