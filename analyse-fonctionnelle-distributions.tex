...
\chapter{Convolution des fonctions et de mesures}\marginnote{24-11-2023}

\section{Convolution des mesures discrètes}

\emph{Comment définir une convolution ?}

\begin{definition}[Mesure discrète]
  On appelle mesure discrète une mesure qui s'écrit

  \[\mu = \sum_{i \in \mathbb{N}} \lambda_j \delta _{x_j}, \lambda_j \in \mathbb{R}.\]
\end{definition}

\begin{definition}[Convolution des mesures discrètes]
  Si \(\nu = \displaystyle\sum \mu_k \delta _{y_k}\) est une autre mesure discrète, on écrit

  \[\mu \ast \nu = \sum_{j,k} \lambda_j \mu_j \delta _{x_j + y_k}.\]

  En fait \(\delta _{x_j}\ast \delta _{y_k} = \delta _{x_j + y_k}\) (la loi interne est vérifiée).
\end{definition}

On peut aussi poser \(x_j + y_k = z_i\) et on a (produit de Cauchy)

\[\mu \ast \nu = \sum_i \left(\sum_{x_j + y_k = z_i} \lambda_j \mu_k \right)\delta _{z_i}.\]

Il se peut que cette quantité ne soit pas définie. Si \(\lambda_i \in \mathbb{R}\) pour tout \(i\), on dit que \(\mu\) et \(\nu\) sont convolables.

\begin{exemple}[Loi de Poisson \(\mathcal{P}(\lambda)\) (distribution de probabilité)]
  Elle se définit comme suit :

  \[\mu = \sum_{k \geq 0} e^{-\lambda} \frac{\kappa ^{k}}{k!}\delta_k.\]

  On a \(\operatorname{supp}(\mu) \subset \mathbb{N}\). Si \(\nu\) est une autre mesure de Poisson, on aura \(\operatorname{supp}(\mu + \nu) \subset \mathbb{N}\).
\end{exemple}

\begin{exemple}[Mesures non convolables]

  \

  \begin{enumerate}
    \item On pose \[\mu = \sum_{j \in \mathbb{N}} \delta_j, \nu = \sum_{k \in \mathbb{N}} \delta _{-k}.\]

    Dans ce cas, on a \(\operatorname{supp}(\mu) = \mathbb{Z}_+\) et \(\operatorname{supp}(\nu) = \mathbb{Z}_{-}\). On a \(\alpha_i = \displaystyle  \sum _{j-k = i} 1 = + \infty\). Donc \(\mu\) et \(\nu\) ne sont pas convolables.
    \item Si on prend \(\mu = \displaystyle\sum_{j \geq 0} 2 ^{-j}\delta_j \) (bornée) et \(\nu = \displaystyle\sum_{j \geq 0} 2 ^{j}d _{-j} \) (non bornée), le produit de Cauchy diverge.
  \end{enumerate}

\end{exemple}

\section{Convolution des mesures de Radon}

\begin{definition}
  On rappelle que la mesure de Radon est un élément du dual de l'espace localement convexe \(\mathcal{K}(X)\), donc appartenant à \(\mathcal{M}(X)\). On peut poser \(X = \mathbb{R}^d\) par exemple.
\end{definition}

On cherche un espace plus petit. On prend \(\mathcal{M}_b(\mathbb{R}^d)\) l'espace des mesures de Radon bornées sur \(\mathbb{R}^d\).

\begin{definition}
  On dit que \(\mu, \nu \in \mathcal{M}(\mathbb{R}^d)\) sont convolables si

  \[\varphi \in \mathcal{K}(\mathbb{R}^d) \longrightarrow \varphi(x+y)d \mu(x)\otimes d \nu(y)\]

  est continue sur \(\mathcal{K}(\mathbb{R}^d)\), i. e. pour tout \(K \subset \mathbb{R}^d\), il existe \(C_k\) tel que pour tout \(\varphi \in \mathcal{K}_K(\mathbb{R}^d)\),

  \[\left\lvert \int \varphi(x+y)d \mu(x)\otimes d \nu(y) \right\rvert \leq C_k \sup_{K} \left\lvert \varphi(x) \right\rvert.\]
\end{definition}

\begin{definition}
  On définit le produit de convolution de \(\mu\) et \(\nu\) par

  \[\mu \ast \nu = \langle \mu \ast \nu, \varphi \rangle  = \int \varphi(x+y)d \mu(x)\otimes d \nu(y) = \langle \mu \otimes \nu, s ^{*} \varphi \rangle,\]

  avec \(s ^{*} \varphi(x,y) = \varphi(x+y)\).
\end{definition}
...
\begin{exemple}
  On veut calculer \(\langle \delta _{x_0} \otimes \delta_{x_1},f \rangle \), avec \(f : \mathbb{R}^d \times \mathbb{R}^d \longrightarrow \mathbb{R}\).

  Soit \(\varphi \in \mathcal{K}(\mathbb{R}^d)\), on pose \(f(x,y) = \varphi(x+y)\).
\end{exemple}

\section{Propriétés géométriques sur les supports}

On a le problème suivant : étant donnés \(\mu, \nu \in \mathcal{M}(\mathbb{R}^d)\), quelle est la condition suivante sur les supports pour que \(\mu \ast \nu\) existe ?

\begin{prop}
  Supposons que \(\forall K \in \mathbb{R}^d\),

  \begin{equation}\label{existence-conv}
    \{ (x,y), x \in \operatorname{supp} \mu \text{ et } y \in \operatorname{supp}\nu, x+y \in K\} \text{ est compact.}
  \end{equation} Alors \(\mu \ast \nu\) existe.
\end{prop}

\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.3]{figures/prop-geo-conv.png}
  \caption{Propriétés sur le supports}
  \label{}
\end{figure}

\begin{definition}
  Les supports de \(\mu\) et \(\nu\) sont convolutifs si et seulement si ils vérifient la propriété \ref{existence-conv}.
\end{definition}

\begin{prop}
  On a \(\operatorname{supp}(\mu \ast \nu) \subset \operatorname{supp}\mu + \operatorname{supp}\nu\).
\end{prop}

\begin{exemple}[Contre-exemples]
  ...
  \begin{itemize}
    ...
    \[\mu = \displaystyle\sum_{n \in \mathbb{Z}_+} \delta_n, \nu = \displaystyle\sum_{n \in \mathbb{Z}_{-}}.\]

    Le produit de Cauchy diverge.

    \item[$\star$] Si \(d \mu(x) = \mathds{1}_{\{ x \bg 0 \}}dx, d \nu(x) = \mathds{1}_{\{ y \less 0 \}} dy\), alors

    \[\int_{0}^{\infty} dx \int_{- \infty}^{0} \varphi(x+y)dy = +\infty.\]
    \item[$\star$] On prend un cône \(C\) et une surface de Cauchy.
  \end{itemize}
\end{exemple}

\begin{equation}\label{existence-conv-2}
  \exists p(R) \text{ tel que } (x,y)\in \underbrace{\operatorname{supp}\mu}_{F} \times \underbrace{\operatorname{supp}\nu }_{G} \text{ et } x + y \in K \implies \left\lvert x \right\rvert \leq p(R) \text{ et } \left\lvert y \right\rvert \leq p(R).
\end{equation}

\begin{prop}
  On a \ref{existence-conv-2} implique \ref{existence-conv}. Les deux propriétés impliquent que \(\mu\) et \(\nu\) sont convolables. De plus, si \(F\) et \(G\) sont fermés, alors \(F+ G\) est aussi fermé.
\end{prop}

\section{Exemples de mesures convolables}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item  Si \(\mu\) ou \(\nu\) est à support compact, alors \(\mu\) et \(\nu\) sont convolables.
    \item Si \(\mu, \nu\) sont des mesures de Radon sur \(\mathbb{R}^2\) tels que \(\operatorname{supp}\mu = F\) et \(\operatorname{supp}\nu = G\) vérifient \ref{existence-conv-2}, alors \(\mu \ast \nu\) existe. En particulier si \(F = G = C\) cône convexe de sommet 0.
    \item Si \(\mu\) et \(\nu\) sont des mesures de Radon bornées, alors \(\mu \otimes \nu \in \mathcal{M}_b(\mathbb{R}^d)\) et \(\mu \ast \nu\) existe.
    \item Une mesure \(\mu\) bornée est en général convolable avec \(\nu\) quelconque.
  \end{enumerate}

\end{prop}

\section{Convolution des fonctions}\marginnote{28-11-2023}

On a

\[\langle \mu \ast \nu, \varphi \rangle = \iint \varphi(x+y)d \mu(x)\otimes d \nu(y).\]

Si \(\mu\) est de densité \(f \in L ^{1}\), \(\nu\) est aussi de densité \(L ^{1}\). On a alors

\[f \ast g(x) = \int f(x-y)g(y)dy,\]

avec \(d \mu(x) = f(x)dx\), \(d \nu(y) = g(y)dy\). On a alors

\[\langle \mu \ast \varphi \rangle = \int \varphi(x+y)f(x)g(y)dx dy.\]

On pose \(z = x+y, x = x\), alors

\[\operatorname{Jac} = \frac{\partial(z,x)}{\partial(x,y)} = \begin{bmatrix}
  1 & 1 \\
  1 & 0
\end{bmatrix} = -1.\]

On réécrit l'expression de la manière suivante et on obtient :

\[\iint \varphi(z)f(x)g(z-x)dx dz = \int \varphi(z)f \ast g(z)dz.\]

Alors \(\mu \ast \nu =(f\ast g)(z)\) est de densité \(L ^{1}\).



\section{Algèbres de convolution}\marginnote{01-12-2023}

\begin{definition}
  \(X = \mathbb{R}^d, \mathcal{M}(X) = (\mathcal{K}(X))'\) espace de mesures de Radon.

  On appelle \(V \subset \mathcal{M}(X)\) algèbre de convolution si \(V\) est un sous-espace vectoriel stable par convolution \(\ast\) et pour lequel \((V,+,\ast)\) est une algèbre.
\end{definition}

\begin{exemple}
  \(\mu = H(x)f(x), f \in L _{\operatorname{loc}}^{1}(\mathbb{R})\). On a alors

  \[\mu \ast \nu(x) = H(x)\int_{0}^{x}f(x-y)g(y)dy.\]
\end{exemple}

\begin{remark}
  \(V\) est unitaire si et seulement si \(\delta_0 \in V\).  \(L ^{1}\) n'est pas unitaire parce que \(\delta _{0}\) n'est pas dans \(L ^{1}\), mais c'est toujours une algèbre.
\end{remark}

\begin{prop}
  \(\mathcal{M}_b(X)\) muni de \(+\) et \(\ast\) est une algèbre unitaire d'élément unité \(\delta_0\).
\end{prop}

\begin{proof}
  Comme \(\mathcal{M}_b(X)\) est un espace de Banach et que \(\mu \ast \delta_0 = \mu\), il suffit de montrer que \(\left\Vert \mu \ast \nu \right\Vert \leq \left\Vert \mu \right\Vert \left\Vert \nu \right\Vert   \). Or pour tout \(\varphi\) tel que \( \left\Vert \varphi \right\Vert _{\infty} \leq 1\),

  \begin{gather*}
    \langle \mu \ast \nu, \varphi \rangle = \left\lvert \iint \varphi(x+y)d \mu(x) \otimes d \nu(y) \right\rvert = \iint \left\lvert d \mu(x)\otimes d \nu(y) \right\rvert = \left\Vert \mu \right\Vert \left\Vert \nu \right\Vert.
  \end{gather*}
\end{proof}

\section{Convolution de mesures et de fonctions}

On prend \(f \in L ^{1}_{\operatorname{loc}}\).

\begin{remark}
  \( L ^{p}(X) \subset L ^{p}_{\operatorname{loc}}(X) \subset L ^{1}_{\operatorname{loc}}(X)\) pour tout \(p \geq 1\).
\end{remark}

\begin{prop}
  Si \(f,g \in L ^{1}\), les mesures de densité \(f\) et \(g\) \(d \mu(x) = f(x)dx\) et \(d \nu(x) = g(x)dx\) sont convolables et \(\mu \ast \nu\) admet pour densité

  \[h(x) = \int f(x-y)g(y)dy \in \mathscr{L}^{1}\]

  définie presque partout et \(h \in L ^{1}(X)\). De plus,

  \[\left\Vert h \right\Vert _{L ^{1}} \leq \left\Vert f \right\Vert _{L ^{1}} \left\Vert g \right\Vert _{L ^{1}}.\]
\end{prop}

\begin{prop}
  Soient \(f \in L _{\operatorname{loc}}^{1}\) et \(g \in L _{\operatorname{loc}}^{1}\). Alors

  \[h(x) = \int f(x-y)g(y)dy\]

  est définie \(x\) presque partout et \(h \in L_{\operatorname{loc}}^{1}\).
\end{prop}

\begin{proof}
  Soit \(\left\lvert x \right\rvert \leq R\) et \(\operatorname{supp}(g)\subset B(0,\rho)\). On a

  \[\int_{}^{}g(x-y)f(y)dy = \int(\mathds{1}_{B(0,\rho+R)}f)(y)g(x-y)dy.\]
\end{proof}

\begin{prop}
  On pose \(h(x) = \displaystyle\int f(x-y)g(y)dy\).

  \begin{enumerate}
    \item Si \(f \in L ^{1}, g \in L ^{\infty}\), alors \(h\) est unirformément continue et bornée (\(h \in C_b(\mathbb{R}^d)\)).
    \item Si \(f \in L ^{p}, g \in L ^{q}, \frac{1}{p}+\frac{1}{q}=1\) alors \(h \in C_0(\mathbb{R}^d), \left\Vert h \right\Vert _{\infty}\leq \left\Vert f \right\Vert_p \left\Vert g \right\Vert_q\).
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Posons \(h(x) =\displaystyle \int f(x-y)g(y)dy = \displaystyle\int f(y)g(x-y)dy\). On a

    \begin{gather*}
      \left\lvert h(x) \right\rvert \leq \int \left\lvert f \right\rvert \left\Vert g \right\Vert _{\infty}  = \left\Vert g \right\Vert _{\infty}\left\Vert f \right\Vert _{L ^{1}},
    \end{gather*}
...
\begin{exemple}[Equation de Lotka-Volterra]
  Soit \(h \in \mathcal{K}(\mathbb{R})\). On a une équation intégrale

  \begin{equation}
    f(x)+ \int_{0}^{x}K(x-y)f(y)dy = h(x).
  \end{equation}

  On peut résoudre cette équation dans \(\overline{\mathbb{R}^{+}}\).

\end{exemple}

\chapter{Distributions}

\begin{definition}[Distribution]\marginnote{08-12-2023}
  C'est le dual de l'espace \(\mathscr{D}'(\Omega)\), avec \(u_j \longrightarrow u \in \mathscr{D}'\).
\end{definition}


On peut multiplier \(f \in \mathcal{C}^{\infty}\) et \(u \in \mathscr{D}'\), mais par contre si \(u, v \in \mathscr{D}'\), cela n'a pas de sens. On va également s'intéresser aux dérivations des distributions et aux solutions faibles.

\section{Définition, convergence dans \(\mathscr{D}, \mathscr{D}'\)}

\begin{definition}
  Soit \(u\) une forme linéaire sur \(\mathscr{D}\) (pas forcément continue). L'application \(\varphi \longmapsto \langle \varphi, u \rangle \) est linéaire. On dit que \(u\) est une distribution d'ordre \(p \in \mathbb{N}\) si et seulement si pour tout compact \(K \subset \Omega\), il existe \(p \in \mathbb{N}\) (l'ordre de la distribution), il existe \(C \bg 0\), pour tout \(\varphi \in \mathscr{D}_K(\Omega)\),

  \[\left\lvert \langle u, \varphi \rangle  \right\rvert \leq C \sup_{\left\lvert \alpha \right\rvert \leq p} \left\lvert \partial ^{\alpha} \varphi \right\rvert.\]

  L'ordre \(p\) est le plus petit entier \(p\) qui satisfait la définition. On note \(\mathscr{D}(\Omega)\) l'ensemble de ses formes linéaires continues.
\end{definition}

\begin{remark}
  C'est compatible avec le fait que \(\mathscr{D}'(\Omega)\) est le dual topologique de \(\mathscr{D}(\Omega)\).
\end{remark}

\begin{remark}
  Soit \(u\) une forme linéaire sur \(\mathscr{D}(\Omega)\). Alors \(u \in \mathscr{D}'(\Omega)\) si et seulement si :

  \[\forall \varphi_j \in \mathscr{D}(\Omega), \varphi_j \longrightarrow 0 \text{ pour la topologie de } \mathscr{D}(\Omega), \text{ alors } \langle u, \varphi_j \rangle \longrightarrow 0.\]
\end{remark}

\begin{exemple}[De distributions]

  \

  \begin{enumerate}
    \item Les mesures de Radon sur \(\Omega\) sont des distributions avec \(p = 0\).
    \item Les fonctions \(L ^{1}_{\operatorname{loc}}(\Omega)\) sont des distributions.

    \emph{Conséquence : } Tous les espaces \(L ^{p}_{\operatorname{loc}}(\Omega)\) sont inclus dans \(\mathscr{D}'(\Omega)\).

    \item Les solutions \textbf{faibles} des équations à dérivées partielles (EDP) :

    \[\displaystyle\frac{\partial ^2 u }{c ^2\partial t ^2} - \displaystyle\frac{\partial ^2 u }{\partial x ^2} = f(x,t), (x,t) \in \mathbb{R}^2.\]

    Or \(u(x,t) = \displaystyle \frac{1}{2}\mathds{1}_{\{ \left\lvert x \right\rvert \less ct \}}\notin \mathcal{C}^R(\Omega)\). On dit que \(u\) est une solution faible avec \(u \in L ^{1}_{\operatorname{loc}}(r ^2), \forall \varphi \in \mathscr{D}(\Omega)\) et on a

    \[\iint \left(\displaystyle\frac{\partial ^2 u }{c ^2\partial t ^2} - \displaystyle\frac{\partial ^2 u }{\partial x ^2}\right)u(x,t)dx dt = \iint f(x,t)dt dx.\]

    \item \emph{Problème aux limites :} On prend \(\Omega\) un ouvert à bord ouvert régulier. On introduit l'équation de Poisson (au sens fort) :

    \[- \Delta u = f \in L ^2\]

    et au sens faible :

    \[- \int \Delta \varphi \cdot u = \int f \varphi,\]

    avec \(u \in L _{\operatorname{loc}}^{1}\) solution faible.
  \end{enumerate}
\end{exemple}

\section{Convergence d'une suite de distributions dans \(\mathscr{D}'(\Omega)\)}

Il s'agit de la convergence faible (on dit encore ``convergence au sens des distributions'').

\begin{definition}
  On dit que \(u_j \in \mathscr{D}'(\Omega)\) converge vers \(u \in \mathscr{D}'(\Omega)\) si

  \[\forall \varphi \in \mathscr{D}(\Omega), \langle u_j, \varphi \rangle \longrightarrow \langle u, \varphi \rangle.\]
\end{definition}

Le support de \(u\), \(\operatorname{supp}u = \mathcal{O} ^{C}\) où \(\mathcal{O}\) est le plus grand ouvert où \(u = 0\). Alors pour tout \(\varphi \in \mathscr{D}(\mathcal{O}), \langle u, \varphi \rangle  = 0\).
...
\begin{exemple}
  ...
  \begin{enumerate}
    \item \(\operatorname{supp}\delta = \{ 0 \}\) ;
    \item \(\operatorname{supp}dx = \mathbb{R}^d\).
  \end{enumerate}
\end{exemple}

La différence avec les mesures est la suivante :

\begin{itemize}
  \item Pour \(\varphi \in \mathcal{K}(\mathbb{R}^d)\), si \(\varphi(0) = 0\), alors \(\langle \delta, \varphi \rangle  = \varphi(0)= 0\).
  \item Si \(\varphi \in \mathscr{D}(\mathbb{R}^d), \varphi \bigm|_{\operatorname{supp}u} = 0\) n'entraine pas que \(\langle u, \varphi \rangle  =0 \).
\end{itemize}

\section{Multiplication (au sens des distributions) et dérivation}

\subsection{Produit d'une distribution par \(f \in \mathcal{C}^{\infty}\)}

\begin{definition}[Définition et proposition]
  Soit \(f \in \mathcal{C}^{\infty}(\Omega), u \in \mathscr{D}'(\Omega)\), alors \(fu \in \mathscr{D}'(\Omega)\) d'ordre inférieur à celui de \(u\), à savoir \(p\). On définit \(fu \in \mathscr{D}'(\Omega)\) par

  \[\langle fu, \varphi \rangle = \langle u, f \varphi \rangle.\]
\end{definition}

\begin{proof}
  Soit \(K \subset \Omega\) compact, soit \(\varphi \in \mathscr{D}_K(\Omega)\). On a

  \[\left\lvert \langle fu, \varphi \rangle  \right\rvert = \left\lvert \langle u, f \varphi \rangle  \right\rvert \leq C \sup_{\left\lvert \alpha \right\rvert \leq p} \left\lvert \partial \alpha(f \varphi)(x) \right\rvert.\]

  On voudrait plutôt \(\widetilde{C} \sup_{\left\lvert \alpha \right\rvert \leq p} \left\lvert \partial ^{\alpha}\varphi \right\rvert\). Par la formule du produit de Leibniz, on a

  \begin{gather*}
    \partial ^{\alpha}(f \varphi) = \displaystyle\sum_{\gamma \leq \alpha} \binom{\alpha}{\gamma}\partial ^{\alpha - \gamma}f(x)\partial ^{\gamma}\varphi(x).
  \end{gather*}

  \(\gamma \leq \alpha\), \(\gamma_j \leq \alpha_j, \forall j\). On a \(\partial ^{\gamma} = \partial ^{\gamma_1} \dots \partial ^{\gamma_d}\). On écrit

  \[ \binom{\alpha}{\gamma} = \frac{\alpha !}{\gamma ! (\alpha- \gamma)!}= \binom{\alpha_1}{\gamma_1} \dots \binom{\alpha_d}{\gamma_d}.\]
\end{proof}

\begin{prop}
  On suppose \(d =1\). Soit \(u \in \mathscr{D}'(\mathbb{R})\). Alors \(xu = 0 \iff u = C \delta\) avec \(C \in \mathbb{C}\).
\end{prop}

Est-ce que l'on a \(x ^2 u = 0 \implies u = \delta\) ?

Si \(u = \delta, \langle x ^2 u, \varphi \rangle \langle u, x ^2 \varphi \rangle  =0\).

\subsection{Dérivée d'une distribution}

\begin{definition}
  Soit \(u \in \mathscr{D}'(\Omega)\), avec \(\Omega \subset \mathbb{R}^d\). Alors

  \[\left\langle \displaystyle\frac{\partial u }{\partial x_j}, \varphi  \right\rangle = - \left\langle u, \displaystyle\frac{\partial \varphi }{\partial x_j}  \right\rangle.\]
\end{definition}

\paragraph{Motivation}

Soit \(u \in L ^{1}_{\operatorname{loc}}, \langle u, \varphi \rangle  = \displaystyle \int u(x)\varphi(x)dx\). Si \(u \in \mathcal{C}^{\infty}\), alors

\[\left\langle \displaystyle\frac{\partial u }{\partial x_j}, \varphi  \right\rangle = \int_{\mathbb{R}^d} \displaystyle\frac{\partial u }{\partial x_j} \varphi dx = - \int u \displaystyle\frac{\partial \varphi }{\partial x_j}dx \text{ (i.p.p.) } \]

\begin{prop}
  On a \(\displaystyle\frac{\partial u }{\partial x_j}\in \mathscr{D}'(\Omega)\) d'ordre inférieur à \(p+1\).

\end{prop}

Plus généralement, \(\langle \partial ^{\alpha}u, \varphi \rangle = (-1)^{\left\lvert \alpha \right\rvert} \langle u, \partial ^{\alpha}\varphi \rangle  \).

\begin{prop}
  Si \(u_j \longrightarrow u\) dans \(\mathscr{D}'\), alors \(\partial ^{\alpha}u_j \longrightarrow \partial ^{\alpha}u\) dans \(\mathscr{D}'\). En particulier, si \(\displaystyle\sum v_j \longrightarrow u\), alors \(\displaystyle\sum \partial ^{\alpha} v_j \longrightarrow \partial ^{\alpha}u\).
\end{prop}

\begin{thm}[Formule des sauts]
  On pose par exemple \(u = H(x)\) fonction de Heavyside. Elle est dans \(L ^{1}_{\operatorname{loc}}\). On a

  \[\langle u', \varphi \rangle = - \langle u , \varphi' \rangle   = - \int_{0}^{\infty}\varphi'(x)dx = \varphi(0).\]
\end{thm}

\begin{prop}
  Soit \(f : I \subset \mathbb{R} \longrightarrow \mathbb{R}\), \(\mathcal{C}^1\) par morceaux discontinue aux points \(a_1, \dots, a_N \in I\), avec \(\langle Tf, \varphi \rangle = \langle f, \varphi \rangle  \) et

  \[f' = Tf' \displaystyle\sum_{i=1}^{N}\sigma_i \delta _{a_i}.\]
\end{prop}

\begin{prop}
  Soit \(u \in \mathscr{D}'(\mathbb{R})\). Alors \(u' = 0\) si et seulement si \(u = C\).
\end{prop}
...
\chapter{Transformations de Fourier dans \(\mathscr{S}(\mathbb{R}^{d})\) et \(\mathscr{S}'(\mathbb{R}^d)\)} \marginnote{21-12-2023}

On veut calculer les transformations de Fourier des distributions qui ne sont pas des fonctions. On veut également déterminer des solutions fondamentales d'EDP, par exemple

\[\displaystyle\frac{\partial  }{\partial t} - \Delta = P.\]

\section{Rappel : transformation de Fourier dans \(L ^{1}\)}

\subsection{Conventions}

C'est l'opérateur \(\mathcal{F} : L ^{1}(\mathbb{R}^d) \longrightarrow L ^{1}_{\operatorname{loc}}(\mathbb{R}^d)\) tel que

\[\widehat{f}(\xi) = F f (\xi) = \int_{}^{} e^{- i \langle x,\xi \rangle }f(x)dx, f \in L ^{1}, \left\lvert e^{-i \langle x,\xi f(x)\rangle } \right\rvert \in L ^{1}.\]

Si \(g \in L ^{1}(\mathbb{R}^d)\), on peut définir la transformation de Fourier inverse par :

\[\mathcal{F}^{-1} g(x) = \frac{1}{(2 \pi)^{d}} \int e^{ix \xi}g(\xi) d \xi,\]

le problème étant de trouver \(\mathcal{F}\) pour que \(\mathcal{F}^{-1} \circ \mathcal{F} f = f\).

On a

\[\mathcal{F}_{1}f(\xi) = \int e^{- i \langle x,\xi \rangle }f(x)\frac{dx}{(2 \pi)^{d/2}}, \]

alors

\[\mathcal{F}_{1}^{-1}g(x) = \int e^{i x \xi}g(\xi) \frac{d\xi}{(2 \pi)^{d/2}}.\]

\subsection{Continuité de \(\mathcal{F}\)}

\begin{thm}[Riemann-Lebesgue]
  On a  \(\mathcal{F} : L ^{1}\longrightarrow C _{0}(\mathbb{R}^d)\).
\end{thm}

\begin{exemple}
  \[\mathcal{F} \mathds{1}_{[-a,a]}(\xi) = \int_{-a}^{a} e^{ix \xi}dx = \left[\frac{e^{- i x\xi}}{- i \xi}\right]^{a}_{-a} = \frac{e^{-ia \xi}+ e^{ia \xi}}{2 i \xi} \times 2 = \frac{2 \sin(a \xi)}{\xi}.\]
\end{exemple}

\begin{remark}
  Si \(f\) est paire, alors sa transformée de Fourier l'est aussi. Si \(f\) est à support compact, alors sa transformée de Fourier est \(\mathcal{C}^{\infty}\).
\end{remark}

\begin{exemple}
  Soit \(f(x) = H(x)e^{- \lambda x}\). Alors

  \begin{gather*}
    \int_{0}^{\infty} e^{- \lambda x }e^{- i x \xi} dx = \int_{0}^{\infty} e^{-i(\xi - i \lambda)x}dx \frac{e^{-i(\xi - i \lambda)}}{-i (\xi - i \lambda)} \bigm|_{0}^{m} = \frac{1}{i (\xi - i \lambda)}.
  \end{gather*}

  C'est une fonction holomorphe.
\end{exemple}

\begin{exemple}
  Soit \(\mathcal{F}\left(\frac{1}{x ^2 + \lambda ^2}\right)\), avec \(e^{- \lambda \left\lvert x \right\rvert} \longmapsto \frac{1}{x ^2 + \lambda ^2}\). \(\mathcal{F}^{-1}\) est bien définie.

  \[\mathcal{F} \left(\frac{1}{x ^2 + \lambda ^2}\right)(\xi) = \frac{e^{- \lambda \left\lvert x \right\rvert}}{2 \lambda}.\]
\end{exemple}

\subsection{Invertion de \(\mathcal{F}\) dans \(\mathcal{L}^{1}\cap \mathcal{F}^{-1}(L ^{1})\)}

\begin{lemma}
  Pour tout \(\sigma \bg 0\),

  \[F(e^{- \sigma ^2 x ^2/2})(\xi) = (2 \pi)^{d/2}\sigma ^{d}e^{- \xi ^2/2 \sigma ^2}.\]
\end{lemma}

\begin{thm}
  Si \(f \in L ^{1} \cap \mathcal{F}^{-1} L ^{1}\), alors

  \[f = (2 \pi)^{-d}\check{F}F f, \]

  avec \(\check{u}(x) = u(-x)\), l'opérateur de parité.
\end{thm}

\section{Transformation de Fourier dans \(\mathscr{S}(\mathbb{R}^d)\)}

\begin{thm}
  \(\forall f \in \mathscr{S}(\mathbb{R}^d)\),

  \[(- D)^{\alpha}\mathcal{F} f (\xi) = \mathcal{F}(x ^{\alpha}f(x))(\xi),\]

  avec

  \[(i \xi)^{\alpha}\mathcal{F} f(\xi) = \mathcal{F} \left(\displaystyle\frac{\partial ^{\alpha}f }{\partial x ^{\alpha} } \right).\]
\end{thm}

\begin{thm}
  \(\mathcal{F} : \mathscr{S}(\mathbb{R}^d) \longrightarrow \mathscr{S}(\mathbb{R}^d)\) est continue, avec

  \[\mathcal{F}^{-1} = (2 \pi)^{-d} \check{\mathcal{F}}.\]
\end{thm}

\section{Distributions tempérées}

\begin{definition}
  \(u \in \mathscr{S}(\mathbb{R}^d)\) si et seulement si il existe \(C\) tel que

  \[\forall \varphi \in \mathscr{S}(\mathbb{R}^d), \left\lvert \langle u,\varphi \rangle  \right\rvert \leq C \left\Vert \varphi \right\Vert _{\alpha,\beta}.\]
\end{definition}

\subsection{Transformée de Fourier dans \(\mathscr{S}'\)}
