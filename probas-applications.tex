...
\chapter{Convergence des variables aléatoires} \marginpar{02-10-2023}
...
\begin{definition}
  ...
  \[\forall \varepsilon \bg 0, \mathbb{P}(\lvert X_n - X \rvert \bg 0) \underset{n \to \infty }{\longrightarrow} 0, \text{ i.e. } \lim_{n \to \infty} \mathbb{P}(\lvert X_n - X \rvert \bg 0) = 0. \]
\end{definition}
...
\begin{lemma}[De Borel-Cantelli]\label{Borel-Cantelli}
  ...
  \[\mathbb{P}(\lim_{n \to \infty} \sup A_n) = \mathbb{P}\left(\bigcap _{n=1} ^{\infty} \bigcup _{k=n}^{\infty} A_k\right) = 0, \]
...
\begin{proof}
  ...
  \[\sum_{k=1}^{\infty} \mathbb{P}(A_k) \less \infty \implies \sum_{k=1}^{n-1} A_k + \sum_{k=n}^{\infty} A_k \less \infty.\]
\end{proof}

\marginpar{03-10-2023}

\begin{lemma}[De Borel-Cantelli, deuxième]
  Si \(\{ A_n \} \) sont \textbf{indépendantes} et \(\sum_{n=1}^{\infty} \mathbb{P}(A_n) = + \infty \), alors

  \[\mathbb{P}(\lim_{n \to \infty} \sup A_n) = 1.\]
\end{lemma}
...
\begin{exemple}
  ...
  Montrer que \(\lim_{n \to \infty} \sup A_n = \emptyset\), mais \(\sum_{n=1}^{\infty} \mathbb{P}(A_n) =+ \infty\).
\end{exemple}

\begin{proof}
  En TD. On tâchera de montrer que les événements \(A_n\) sont indépendants.
\end{proof}

\begin{exemple}
  Soit \(\{ X_n \}_{n=1}^{\infty}\) une suite de variables aléatoires telles que \(\mathbb{P}(X_n=0)=\frac{1}{n ^2}\). Donc par Borel-Cantelli on sait que

  \[\sum_{n=1}^{\infty} \mathbb{P}(X_n = 0)=\sum_{n=1}^{\infty} \frac{1}{n ^2} \less \infty\]

  et \(\mathbb{P}(\lim_{n \to \infty} \sup \{ X_n = 0 \}) = 0\). Interpréter le résultat.
\end{exemple}
...
Si \(\{ X_n \} \) est une suite de variables aléatoires, on considère \(S_n = \frac{1}{n} \sum_{k=1}^{n} X_k\) (somme de Cèsaro).

\paragraph{Cadre} On a une suite de variables aléatoires \(\{ X_n \} _{n=1}^{\infty}\) sur \((\Omega, \mathscr{A}, \mathbb{P})\). On considère \(S_n = \frac{1}{n}\sum_{k=1}^{n} X_n\).

\subsection{Loi faible des grands nombres}


\begin{definition}
  Supposons que les variables aléatoires \(X_n\) soient centrées, i. e. \(X_n \text{ devient }  X_n - \mathbb{E}[X_n]\) (donc les \(X_n\) auront l'espérance nulle). On dira que la suite \(\{ X_n \}\) satisfait \textbf{la loi faible des grands nombres} si la suite \(S_n\) converge vers 0 \textbf{en probabilité}, c'est-à-dire
...
\begin{prop}\label{conv-L2}
  ...
  \[\mathbb{V}ar(X_n) = \sigma_n ^2 \less \infty \]

  et \[\frac{1}{n ^2}\sum_{k=1}^{n} \sigma_k ^2 \underset{n \to \infty}{\longrightarrow} 0. \]
...
\begin{remark}[Rappel]
  ...
  \[\mathbb{V}\operatorname{ar}(X) = \mathbb{E}[X ^2] - (\mathbb{E}[X])^2. \]
\end{remark}

\begin{prop}
  \begin{enumerate}
    \item \(\mathbb{V}\operatorname{ar}(\alpha X) = \alpha ^2 \mathbb{V}\operatorname{ar}(X)\).
  \end{enumerate}
\end{prop}

\begin{remark}
  \[\mathbb{V}\operatorname{ar}\left(\sum_{k=1}^{n} X_k \right) = \sum_{k=1}^{n} \mathbb{V}\operatorname{ar}(X_k) + 2 \sum_{1 \leq  j \leq k \leq n} \operatorname{Cov}(X_j, X_k).\]
\end{remark}
...
\begin{proof}[Démonstration de la proposition \ref{conv-L2}]
  ...
  On a \begin{gather*}
    \left\Vert \frac{1}{n} \sum_{k=1}^{n} X_k  \right\Vert _{L ^{2}}^{2} = \mathbb{V}\operatorname{ar} \frac{1}{n} \sum_{k=1}^{n} X_k = \frac{1}{n ^2} \mathbb{V}\operatorname{ar}\left(\sum_{k=1}^{n} X_k \right) = \frac{1}{n ^2} \sum_{k=1}^{n}\mathbb{V}\operatorname{ar}(X_k),
  \end{gather*}

  car les variables aléatoires sont non-corrélées.

  De plus, on a \[\frac{1}{n ^2} \sum_{k=1}^{n} \mathbb{V}\operatorname{ar}(X_k) = \frac{1}{n ^2}\sum_{k=1}^{n} \sigma_k ^2 \underset{n \to \infty}{\longrightarrow} 0. \]
