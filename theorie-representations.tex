...
\begin{enumerate}
  ...
  \begin{gather*}
    ...
    \langle v,v \rangle \geq 0 \text{ et } \langle v,v \rangle = 0 \iff v = 0.
  \end{gather*}
...
\begin{exemple}
  ...
  \begin{equation*}\label{hydrogene}
    -\sum_{j=1}^{3} \frac{\partial ^2 \psi(x) }{\partial x_j ^2 }  - \frac{1}{\left\lvert x \right\rvert}\psi(x) = E \psi(x).\tag{$\star$}
  \end{equation*}
\end{exemple}
...
\begin{prop} \marginnote{13-11-2023}
  Les groupes suivants sont les groupes de Lie matriciels :

  \begin{enumerate}
    \item \(GL(n;\mathbb{C})\) ;
    \item \(GL(n;\mathbb{R})\) ;
    \item \(SL(n;\mathbb{C}) = \{ A \in GL(n,\mathbb{C}) \mid \operatorname{det}(A) = 1 \} = \operatorname{det}^{-1}(\{ 1 \})\) (qui est un fermé de \(GL(n;\mathbb{C})\)) ;
    \item \(SL(n;\mathbb{R}) = \{ A \in GL(n;\mathbb{R}), \operatorname{det}(A) = 1 \}\) ;
    \item \(U(n)  =\{ A \in GL(n,\mathbb{C}), A ^{*}A = \operatorname{id} \}\) ;
    \item \(SU(n) = \{ A \in U(n), \operatorname{det}(A) = 1 \}\) ;
    \item \(O(n) = \{ A \in \mathbb{M}(n;\mathbb{R}), A ^{t}A = \operatorname{id} \}\) ;
    \item \(SO(n) = \{ A \in O(n), \operatorname{det}(A) = 1 \}\) ;
    \item Groupe de Heisenberg

    \[H = \left\{ A \in \mathbb{M}(3;\mathbb{R}), A = \begin{pmatrix}
      1 & x & y \\
      0 & 1 & z \\
      0 & 0 & 1
    \end{pmatrix} \text{ pour } x,y,z \in \mathbb{R}  \right\}.\]
    \item Soient \(p,q \in \{ 0, \dots, n \} \text{ tels que } p+q = n\).
    Le groupe unitaire généralisé

    \[U(p,q) = \left\{ A \in \mathbb{M}(n,\mathbb{C}), A ^{*} \begin{pmatrix}
      1 & 0 & \dots & 0 & 0 \\
      0 & 1 & \dots & 0 & 0 \\
    ...
    \(\text{ où } 1 \text{ apparait } p \text{ fois } et -1 \ q \text{ fois }\). On a aussi

    \[SU(p,q) = \{ 1 \in U(p,q), \operatorname{det}(A) = 1\}.\]

    \item Le groupe orthogonal généralisé :

    \[O(p,q) = \left\{ A \in \mathbb{M}(n;\mathbb{R}) = A ^{t}\begin{pmatrix}
      1 & 0 & \dots & 0 & 0 \\
      0 & 1 & \dots & 0 & 0 \\
      \vdots & \vdots & \ddots & \vdots & \vdots \\
      0 & 0 & \dots & -1 & 0\\
    ...
    \(\text{ où } 1 \text{ apparait } p \text{ fois } et -1 \ q \text{ fois }\).

     Dans le cas de \(O(3,1)\), on parle de \textbf{groupe de Lorentz}.
    \item Le groupe symplectique

    \[Sp(2n;\mathbb{R}) = \left\{ A \in \mathbb{M}(2n,\mathbb{R}), A ^{t} \begin{pmatrix}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    Donc c'est un sous-groupe. On a

    \[H = \mathbb{M}(3;\mathbb{R}) \cap p _{11}^{-1}(\{ 1 \}) \cap p _{22}^{-1}(\{ 1 \}) \cap p _{33}^{-1}(\{ 1 \}) \cap p _{31}^{-1}(\{ 0 \}) \cap p _{21}^{-1}(\{ 0 \}) \cap p _{32}^{-1}(\{ 0 \}).\]
...
\begin{prop}
  Les groupes suivants sont isomorphes à des groupes de Lie matriciels :

  \begin{enumerate}
    \item \textbf{Le groupe euclidien :}

    \[E(n) := \{ f : \mathbb{R}^n \longrightarrow \mathbb{R}^n, f(x) = Ax+b \text{ avec } b \in \mathbb{R}^2, A \in O(n) \}.\]

    \item \textbf{Le groupe de Poincaré :}

    \[P(n,1) := \{ f \in \mathbb{R}^{n+1} \longrightarrow \mathbb{R}^{n+1}, f(x) = Ax+b , b \in \mathbb{R}^{n+1}, A \in O(n,1)\}.\]

    \item \((\mathbb{R}^{*}, \times) \simeq GL(1,\mathbb{R})\) ;
    \item \((\mathbb{C},\times) \simeq GL(1,\mathbb{C})\) ;
    \item \(S ^{1} = \{ z \in \mathbb{C}, \left\lvert z \right\rvert = 1 \} \simeq \varphi(1)\).
    \item \((\mathbb{R}, +) \simeq GL(1, \mathbb{R})^{+} = \{ A \in \mathbb{M}(1;\mathbb{R}), \operatorname{det}(A) \bg 0 \}\).
  \end{enumerate}
\end{prop}

\section{Propriétés topologiques des groupes de Lie matriciels}

Nous nous intéressons aux propriétés suivantes : compacité, connexité, supplémentaire connexe.

\begin{definition}
  Un groupe de Lie matriciel est \textbf{compact} s'il est un sous-ensemble compact de \(\mathbb{M}(n;\mathbb{C})\).
\end{definition}
...
fonction
  \begin{enumerate}
    \item La topologie de \(\mathbb{M}(n;\mathbb{C})\) est celle d'un espace vectoriel normé, donc \(G\) est compact si et seulement si \(G\) est fermé et borné.
    \item \(G\) compact si et seulement si \(\forall (A^{(m)})_{m \in \mathbb{N}}\subset G\). Si \(\lim A ^{(m)}=A\), alors \(A \in G\) et \[ \exists c \bg 0, \forall A \in G, \forall j, k \in \{1, \dots, n \}, \left\lvert A _{j,k} \less c \right\rvert.\]
  \end{enumerate}
\end{prop}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(O(n), SO(n), U(n), SU(n)\) sont compacts.
    \item \(SL(n,\mathbb{C})\) si \(n \geq  2\), \(Sp(2n,\mathbb{R}),GL(n,\mathbb{K})\) et \(O(p,q)\) si \(p \neq q \neq 0\) ne sont pas compacts.
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item On a vu que ce sont des fermés de \(\mathbb{M}(n;\mathbb{C})\) et les colonnes sont de longueur 1, donc bornés.
    \item On a \(A \in \begin{pmatrix}
      x & 0 & \dots & 0 \\
      0 & \frac{1}{x} & \dots & 0 \\
      0 & 0 & 1 & \dots \\
      \vdots & \vdots & \ddots & \vdots
    \end{pmatrix} \in SL(n,\mathbb{C})\) pour \(x \neq 0\). Donc si \(SL(n,\mathbb{C})\) n'est pas borné, alors \(GL(n,\mathbb{K})\) non borné. Les autres groupes sont donnés en exercice.

    Remarque importante : \(Sp(2,\mathbb{R}) = SL(2,\mathbb{R})\).
  \end{enumerate}
\end{proof}
...
\begin{definition}
  ...
  \begin{enumerate}
    \item Un groupe de Lie matriciel \(G\) est \textbf{connexe} (par arcs) si

    \[\forall A, B \in G, \exists c \in \mathcal{C}^{0}([0,1],G), c(0)=A, c(1) = B.\]

    \item La composante connexe de l'identité est

    \[G_0 := \{ A \in G, \exists c \in \mathcal{C}^{0}([0,1], G), c(0) = \operatorname{id}, c(1) = A \}.\]

    Les groupes de Lie matriciels connexes par arcs sont connexes (ce sont deux notions équivalentes pour les groupes de Lie).
  \end{enumerate}

\end{definition}

\begin{prop}
  \(G_0\) est un sous-groupe distingué.
\end{prop}

\begin{proof}
  La multiplication de groupe et l'inverse étant continues, on a, pour \(A,B \in G_0\), \(c_A,c_B\) des arcs connectant à l'identité :

  \[t \longmapsto (c_A(t)) ^{-1} \in \mathcal{C}^0, c_A(t) ^{-1} \bigm|_{t=0} = \operatorname{id}, (c_A(1))^{-1} = A ^{-1}.\]

  De plus, on a

  \[t \longmapsto c_A(t)c_B(t)\in \mathcal{C}^0, c_A(t)c_B(t) \bigm|_{t=0} = \operatorname{id}, c_A(1)c_B(1) = AB,\]

  donc \(G_0\) est un groupe. Pour \(B \in GL(n,\mathbb{K})\), on a

  \begin{gather*}
    t \longmapsto B c_A(t)B ^{-1} \in \mathcal{C}^0, B c_A(0)B ^{-1} = \operatorname{id}, B c_A(1) B ^{-1} = B A B ^{-1},
  \end{gather*}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item L'image d'un connexe par une fonction continue est connexe (TVI généralisé), donc \(A G_0\) est connexe pour \(A \in G\). C'est la composante connexe de \(A\).
    \item \(G/G_0\) est un groupe (groupe des composantes connexes).
    \item \(\mathbb{C}^{*} = GL(1,\mathbb{C})\) est connexe.
    \item De même, \(U(1)\subset GL(1;\mathbb{C})\) est connexe.
    \item \(GL(1;\mathbb{R})\subset GL(1;\mathbb{C})\) n'est pas connexe (2 composantes) ;
    \item \(O(2)\) a deux composantes connexes (TD).
    \item \(O(1,1)\) a deux composantes connexes (TD).
  \end{enumerate}
\end{prop}

\begin{prop}
  \(GL(n,\mathbb{C})\) est connexe \(\forall n \in \mathbb{N}\).
\end{prop}

\begin{proof}
  Soit \(A \in GL(n;\mathbb{C})\) et \(C \in GL(n;\mathbb{C})\) tels que \(C ^{-1} A C\) soit une matrice triangulaire. Les \(\lambda_j\) sont les valeurs propres, \(\lambda_j \in \mathbb{C}^{*}\) qui est connexe. Soit \(\lambda_j(\cdot) \in \mathcal{C}^0([0,1],\mathbb{C}^{*}), \lambda_j(0)=1, \lambda_j(1) = \lambda_j\).
  ...
  \vdots & \ddots & \vdots \\
  0 & \dots & \lambda_n(t)
  \end{pmatrix}C ^{-1}+t C N C ^{-1},\]

  alors \(A(\cdot) \in \mathcal{C}^0([0,1], GL(n,\mathbb{C})), A(0) = \operatorname{id},A(1) = A\). Donc \(GL(n,\mathbb{C})\) est connexe.
\end{proof}

\begin{prop}
  Pour \(A \in \mathbb{M}(n;\mathbb{C})\), il existe \(C \in GL(n;\mathbb{C})\) tel que \(C ^{-1} A C \) est triangulaire supérieure.
\end{prop}
...
\begin{corollary}
  ...
  \begin{enumerate}
    \item Les éléments diagonaux sont les valeurs propres.
    \item \(C\) peut être choisie unitaire.
  \end{enumerate}
\end{corollary}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(SL(n,\mathbb{C})\) est connexe.
    \item \(U(n), SU(n)\) sont connexes.
    \item \(SO(n)\) est connexe.
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Dans la preuve pour \(GL(n,\mathbb{R})\), choisir les mêmes \(\lambda_n(1), \dots, \lambda _{n-1}(t)\) et \(\lambda_n(t):=\displaystyle \frac{1}{\lambda_1(1)\dots \lambda _{n-1}(t)}\).

    Alors \(\lambda_n(\cdot) \in \mathcal{C}^0, \lambda_1(0)= 1, \lambda_n(1)=\lambda_n\), car \(\lambda_1 \dots \lambda_n = \operatorname{det}(A) = 1\) et \(\operatorname{det}(A(t)) = 1,\forall t\).

    \item \(A \in U(n)\) est unitairement diagonalisable et ses valeurs propres sont de \(U(1)\).

    On pose \[A = C \begin{pmatrix}
    e^{i \theta_1} & \dots & 0 \\
    \vdots & \ddots & \vdots \\
    0 & \dots & e^{i \theta_n}
    \end{pmatrix}.\]

    \(t \longmapsto A(t)\) est continue, \(A(t) \in U(n)\), \(A(0) = \operatorname{id}, A(1) = 1\), donc \(U(n)\) est connexe.

    Pour \(A \in SU(n)\), on remplace \(e^{i \theta_n t}\) par \(e^{-it(\theta_1 + \dots + \theta _{n-1})}\).
  \end{enumerate}
\end{proof}

\begin{definition}\marginnote{15-11-2023}
  Soit \(G\) un espace topologique connexe par arcs. On dit que \(G\) est simplement connexe si pour tout lacet \(C \in \mathcal{C}^0([0,1], G), c(0)=c(1)\), il existe \(H_C \in \mathcal{C}^0([0,1]\times[0,1], G)\) tel que

  \begin{enumerate}
    \item \(H_C(S, 0) = H_C(S,1)\) pour tout \(S\) (famille de lacets),
    \item  \(H_C(0,t) = C(t)\) pour tout \(t\) (départ \(C\)),
    \item \(H_S(1,t) = H_C(1,0)\) pour tout \(t\) (arrivée en un point).
  \end{enumerate}

  On dit alors que \(H_C\) est une homotopie qui pour chaque lacet \(S\) déforme le lacet \(C\) en un point, ou encore que \(C\) est une homotopie en un point.
\end{definition}

\begin{exemple}
  On pose \(G = \mathbb{C} \setminus \{ 0 \}\), \(c(t) = 2 + \cos(2 \pi t) + i \sin(t)\) et \(H_c(s,t) = (2 + (1-s)\cos(2 \pi t))+i(1-s)\sin(2 \pi)\). C'est un lacet pour chaque \(s\).

  \begin{enumerate}
    \item \(H_c(0,t) = c(t)\) ;
    \item \(H_c(1,t) = 2\) (lacet constant).
  \end{enumerate}
\end{exemple}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(\mathbb{R}^n\) est simplement connexe.
    \item \label{c_priv_0_simpl_connx} \(\mathbb{C} \setminus \{ 0 \}\) n'est pas simplement connexe.
    \item \(U(1)\) n'est pas simplement connexe.
    \item \(SO(2)\) n'est pas simplement connexe.
    \item \(U(n), GL(n;\mathbb{C})\) ne sont pas simplement connexes.
    \item \(SU(2)\) est simplement connexe.
    \item \(SO(3)\) n'est pas simplement connexe.
  \end{enumerate}
\end{prop}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(z \longmapsto e^{z}= \displaystyle \sum_{n=0}^{\infty} \displaystyle \frac{z ^{n}}{n} \in \mathcal{C}^0(\mathbb{C}, \mathbb{C} \setminus \{ 0 \})\).
    \item Il existe \(e \in \mathcal{C}^0(\mathbb{C} \setminus (-\infty, 0], \mathbb{C})\) tel que

    \[\forall z \in \mathbb{C} \setminus (-\infty, 0], e \circ p \circ e(z)=z.\]
  \end{enumerate}
\end{prop}

\begin{proof}
  On a \(e(z) = \log(z)+i \theta \left(\displaystyle\frac{z}{\left\lvert z \right\rvert}\right)\), avec \(\log(r) = \displaystyle\int_{1}^{r} \displaystyle \frac{1}{x}dx \).
\end{proof}

\begin{prop}
  Soit \(c \in \mathcal{C}^0([0,1], \mathbb{C}), c(0)=c(1)\) un lacet et \(p \in \mathbb{C} \setminus \{ 0 \}\). S'il existe une homotopie qui déforme \(c\) en le point \(p\), alors

  \[c(t) = p e^{i 2 \pi \theta(t)},\]

  avec \(g \in \mathcal{C}^0([0,1], \mathbb{C}), g(1) = g(0)\).
\end{prop}

\begin{corollary}
  \(c(t) = e^{i 2 \pi t}\) ne peut pas être déformé en \(p\).
\end{corollary}

\section{Homomorphismes}

\begin{definition}
  Soient \(G,H\) deux groupes de Lie.

  \begin{enumerate}
    \item \(f : G \longrightarrow H\) est appelé homomorphisme de groupes de Lie si

    \[\forall A,B \text{ et } f \in \mathcal{C}^0(G,H), f(AB) = f(A)f(B).\]

    \item Si un homomorphisme \(f\) est bijectif et \(f ^{-1} \in \mathcal{C}^0\), (donc \(f\) est un homéomorphisme), alors \(f\) est appelé \textbf{isomorphisme de groupes de Lie}.
  \end{enumerate}
\end{definition}

\begin{prop}[Propriété]
  On va démontrer ultérieurement que \(G,H\) sont des sous-variétés et que \(f \in \mathcal{C}^{\infty}\).
\end{prop}
...
\begin{exemple}
  ...
  \begin{enumerate}
    \item \(\operatorname{det} : GL(U,\mathbb{C}) \longrightarrow \mathbb{C}^{*}\) est continu et \(\operatorname{det}(AB)=\operatorname{det}(A)\operatorname{det}(B)\).
    \item \(f: \mathbb{R} \longrightarrow U(1)\), \(f(x) = e^{2 \pi i x}\) homéomorphisme et \(f(x+y) = f(x)+f(y)\), donc c'est un homéomorphisme de groupes de Lie.
    \item \(f : U(1) \longrightarrow SO(2), f(z) = \begin{pmatrix}
    \Re(z) & - \Im(z) \\
    \Im(z) & \Re(z)
    \end{pmatrix}\).
  \end{enumerate}
...
Un exemple important est l'homomorphisme \(SU(2) \longrightarrow SO(z)\) qui n'est pas un isomorphisme.

\begin{prop}
 Soit

    \[V= \{ X \in \mathbb{M}(z,\mathbb{C}), X = X ^{*}, \operatorname{Tr}(X) = 0 \}\]

    et \(\langle X,Y \rangle = \operatorname{Tr}(X ^{*} Y)\) le produit scalaire.

    Soit, pour \(U \in SU(2)\), \(f(U) : V \longrightarrow V\), \(f(U)(X) = U ^{*} X U\) (on aura \(\operatorname{Tr}(f(U)(X)) = \operatorname{Tr}(U ^{*} X U) = \operatorname{Tr}(U U ^{*} X) = \operatorname{Tr}(X)\)). Alors

    \[\forall X,Y \in V, \langle f(U)(X),f(U)(Y) \rangle = \langle X,Y \rangle.\]

    \(f : SU(2) \longrightarrow O(V)\) est un homomorphisme surjectif sur \(SO(V)\) tel que \(\operatorname{Ker}(f) = \{ \operatorname{id}, -\operatorname{id} \}\).

    \(V\) est un espace vectoriel réel de dimension 3. Soit pour une base orthonormée \(\{ b_1, b_2, b_3 \}\),

    \[i : V \longrightarrow \mathbb{R}^3, i \left(\sum_{j=1}^{3}x_j b_j \right) = (X_1, X_2, X_3).\]

    \(SU(2) \in A \longrightarrow i \circ f(A) \circ i ^{-1} \longrightarrow SO(3)\) est un homomorphisme surjectif de \(\operatorname{Ker}\{ \operatorname{id},-\operatorname{id} \}\).
\end{prop}
...
\begin{proof}
  ...
  \begin{gather*}
    f(U)(\lambda X +Y) = \lambda f(U)X + f(U)Y, \\
    (f(U)(X))^{*} = U ^{*} X U = f(U)(X), \\
    \langle f(U)X, f(U)Y \rangle  = \operatorname{Tr}(U ^{*} X U U ^{*} Y U) = \operatorname{Tr}(X ^{*} Y) = \langle X,Y \rangle, \\
    f(U)f(V) = f(UV), f(U ^{*}) = (f(U))^{-1}.
  \end{gather*}

  \(f \in \mathcal{C}^0\), car \(A \in \mathbb{M}(2,\mathbb{C}) \longrightarrow A ^{*} X A\) est une composée de fonctions continues. Pour tout \(X \in \mathbb{M}(2,\mathbb{C}), SU(2)\) est connexe, donc \(f(SU(2))\) est connexe et \(\operatorname{id} = f(\operatorname{id}(SU(2)))\), donc \(f(SU(2))\subset SO(V)\), car c'est le plus petit ensemble connexe qui contient \(\operatorname{id}\). On démontrera la surjectivité en TD.

  De plus, on a

  \begin{gather*}
    f(U) = \operatorname{id} \iff U ^{*} X U \iff U U ^{*} X U = U X, \forall X \in V.
  \end{gather*}
\end{proof}
...
\begin{definition}
  Un groupe de Lie est une \textbf{variété différentielle} qui est un groupe tel que le produit \(G \times G \longrightarrow G\) et l'inverse \((\cdot)^{-1} : G \longrightarrow G\) sont des \textbf{fonctions différentiables}.
\end{definition}
...
\begin{prop}[Propriétés]
  ...
  \begin{enumerate}
    \item Les groupes de Lie matriciels sont des groupes de Lie.
    \item Il existe des groupes de Lie tels qu'il n'existe aucun homomorphisme injectif et continu à valeurs dans \(GL(U,\mathbb{C})\).
  \end{enumerate}
\end{prop}
...
\begin{exemple}
 ...
 \((a,b,c) \cdot(x,y,z) = a+x,b+y,e^{iay}\subset z\) (calcul à faire).
\end{exemple}

\chapter{Algèbre de Lie}

\section{Exponentielle et logarithme des matrices}

\begin{prop}
  Pour \(A \in \mathbb{M}(n,\mathbb{C})\),

  \[\exp(A) = e^{A} := \sum_{m=0}^{\infty} \frac{A ^{m}}{m!} \]

  est bien définie et \(\exp \in \mathcal{C}^0(\mathbb{M}(n,\mathbb{C}), \mathbb{M}(n,\mathbb{C}))\).
\end{prop}

\begin{proof}
  On peut choisir la norme \(\left\Vert A \right\Vert_F \) pour laquelle on a \(\left\Vert A ^{m}\right\Vert_F \leq \left\Vert A \right\Vert ^{m}_F  \), donc \(\sum_{0}^{\infty}\frac{\left\Vert A \right\Vert ^{m}}{m!}\) est convergente, donc \(\sum_{0}^{\infty}\frac{A ^{m}}{m!}\), car \(\mathbb{M}(n,\mathbb{C})\) est complet.

  Pour chaque \(N, f_N(A):= \sum_{m=0}^{N}\frac{A ^{m}}{m!} \in \mathcal{C}^0\), car polynôme des projections et

  \[\sup_{\left\Vert A \right\Vert_F \less R}\left\Vert f_N(A)-f_M(A) \right\Vert  \leq \sum_{N}^{M} \frac{R ^{m}}{m!} \underset{N,M \to \infty}{\longrightarrow} 0.\]

  \(\mathcal{C}^0(\mathbb{M}(n,\mathbb{C}), \mathbb{M}(n,\mathbb{C}))\) est complet, donc \(\exp \in \mathcal{C}^0\).
\end{proof}

\begin{prop}\marginnote{20-11-2023}
  Pour \(A, B \in \mathbb{M}(n,\mathbb{C})\),

  \begin{enumerate}
    \item \([A,B]=0 \implies e^{A}e^{B}= e^{A+B}\).
    \item Pour \(C \in GL(n,\mathbb{C}), C e^{A}C ^{-1} = e^{C A C ^{-1}}\).
    \item \(e^{0} = \operatorname{id} = A\).
    \item \(e^{A ^{*}}=(e^{A})^{*}\).
    \item \(\exp : \mathbb{M}(n,\mathbb{C}) \longrightarrow GL(n,\mathbb{C})\) et

    \[(e^{A})^{-1} = e^{-A}.\]

    \item \(e^{(\lambda+\mu)A} = e^{\lambda A}e^{\mu A}, \forall \lambda, \mu \in \mathbb{C}\).
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Si \([A,B]= AB-BA = 0\), alors

    \[(A+B)^{m} = \sum_{0}^{m} \frac{m!}{j!(m-j)!}A ^{j}B ^{m-j}.\]

    \item Argument principal : continuité de la multiplication.
    \item
    \item L'opération \(A \longmapsto A ^{*}\) est continue.
    \item \([-A,A]=0\), doc \(e^{-A+A}=e^{0} = 1 = e^{-A}e^{A}\).
    \item \([\lambda A, \mu A] = 0\).
  \end{enumerate}
\end{proof}
...
\begin{remark}
  ...
  0 & 0 \\
  1 & 0
  \end{pmatrix}\), on a \(A ^2 = B ^2 = 0\) (matrices nilpotentes).
  ...
  \begin{gather*}
    ...
    1 & t \\
    0 & 1
  \end{pmatrix}, e^{tB} = \begin{pmatrix}
  1 & 0 \\
  t & 1
\end{pmatrix}, e^{tA} e^{tB} = \begin{pmatrix}
1 + t ^2 & t \\
t & 1
\end{pmatrix} \neq e^{t(A+B)} \text{ (preuve en TD). }
  \end{gather*}

\end{remark}

\begin{prop}
  Pour \(A \in \mathbb{M}(n,\mathbb{C}), c(t):=e^{tA}, t \in \mathbb{R}\), on a \(c \in \mathcal{C}^1(\mathbb{R}, \mathbb{M}(n,\mathbb{C}))\) et \(\frac{d}{dt}c(t) = A c(t), c(0) = \operatorname{id}\).
\end{prop}

\begin{proof}
  Pour tous \(j,k\), \((e^{tA})_{jk}\) est une série entière avec

  \[\frac{d}{dt} \sum = \sum \frac{d}{dt}.\]
\end{proof}
...
\begin{remark}
  ...
  Exemple avec les matrices définies dans la remarque précédente.
\end{remark}

\begin{remark}[Calcul de \(\exp(A)\) en pratique]
  ...
  \begin{enumerate}
    ...
    \item Si \(A\) est nilpotente avec \(A ^{j} = 0\), alors

    \[e^{A} = 1 + A + \frac{1}{2!} A ^2 + \dots + \frac{1}{(j-1)!}A ^{j-1}.\]
    \item Pour \(A \in \mathbb{M}(n,\mathbb{C})\), il existe \(S\) diagonalisable et \(N\) nilpotente telles que \([S,N] = 0\) et \(A = S+N\), alors \(e^{A} =e^{S}e^{N}\).
  \end{enumerate}
\end{remark}
...
\section{Logarithme matriciel}

\begin{remark}
  \(z \in \mathbb{C} \longmapsto \exp z \in \mathbb{C} \setminus \{ 0 \}\) est surjective, elle n'est pas injective, c'est un logarithme continu qui ne peut être défini que sur \(\mathbb{C} \setminus (-\infty, 0]\).
\end{remark}
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item \(\log(z) := \displaystyle \sum_{1}^{\infty}- \frac{1}{m}(1-z)^{m}\), avec \(z \in B(1,1)\), est analytique et \(e^{\log(z)}  = z\).
    \item \(\exp(B(0,\log(z))) \subset B(1,1)\) et \(\log(e^{z}) = z\) (\(z \in B(0, \log(z))\)).
  \end{enumerate}
\end{lemma}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Pour \(x \in \mathbb{R}, \left\lvert x \right\rvert \less 1\),

    \[\log(x) = \int_{1}^{x} \frac{1}{1-(1-y)}dy = \int_{1}^{x} \sum_{n=0}^{\infty} (1-y)^{n}dy = -\sum_{n=0}^{\infty}  \frac{(1-x)^{n}}{n}\]

    par analycité pour \(z \in \mathbb{C}\).
    \item On a

    \[\left\lvert e^{z}-1 \right\rvert \leq \sum_{n=1}^{\infty} \frac{\left\lvert z \right\rvert ^{n}}{n!} \leq e^{\left\lvert z \right\rvert}-1 \less z-1\]

    si \(\left\lvert z \right\rvert \leq \log (z)\). On a \(\log(e^{z})=z\) pour \(z \in B(0, \log(z)) \cap \mathbb{R}\) et \(e^{\log(z)}=z\) pour \(B(1,1)\cap \mathbb{R}\), donc par analycité c'est vrai partout.
  \end{enumerate}
\end{proof}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item Soit \(B(\operatorname{id},1) = \{ A \in \mathbb{M}(n,\mathbb{C}), \left\Vert A - \operatorname{id} \right\Vert \less 1 \}\).

    \begin{gather*}
      \log A = \sum_{n=1}^{\infty} - \frac{1}{n}(\operatorname{id}-A)^{n},
    \end{gather*}

    alors \(\log \in \mathcal{C}^0(B(\operatorname{id},1), \mathbb{M}(n,\mathbb{C}))\) et \(e^{\log A} = A, \forall A \in B(\operatorname{id},1)\).
    \item Soit \(B(0, \log z ) :=\{ A \in \mathbb{M}(n,\mathbb{C}), \left\Vert A \right\Vert _{F} \less \log z  \}\), alors \(\exp(B(0,\log z))\subset B(\operatorname{id},1)\) et \(\log e^{A} = A, \forall A \in B(0, \log z)\).
  \end{enumerate}
\end{prop}

\begin{prop}[Propriété]
  Pour \(A  = 2 \pi i \operatorname{id}, e^{A} = \operatorname{id} \in B(\operatorname{id},1)\), mais \(\log e^{A} = 0 \neq A\).
\end{prop}
...
\begin{thm}[Formule de produit de Lie]\label{prod-lie}
  Pour \(A,B \in \mathbb{M}(n,\mathbb{C})\),
...
\begin{lemma}
  Il existe \(c \bg 0\) tel que \(\forall B \in B(0,\frac{1}{2}) \subset \mathbb{M}(n,
  \mathbb{C})\), \(\left\Vert \log(\operatorname{id}+B) - B \right\Vert \leq c \left\Vert B \right\Vert ^2\).
\end{lemma}

\begin{proof}
  \(\operatorname{id}+B \in B(\operatorname{id},1)\) et

  \[\log(\operatorname{id}+B) - B = \sum_{m=2}^{\infty} \frac{(-1)^{m+1}}{m} B ^{m} = B ^2 = \sum_{2}^{\infty}\frac{(-1)^{m+1}}{m}B ^{m-2}.\]

  De plus, on a

  \[\sum_{2}^{\infty} \left\Vert \frac{B ^{m-2}}{m} \right\Vert  \less \sum \frac{1}{2 ^{m-2}} =  c \less \infty.\]
\end{proof}

\begin{proof}[Démonstration du théorème \ref{prod-lie}]
  On a

  \[e^{X/m}e^{Y/m} = \operatorname{id} + \frac{X}{m} + \frac{Y}{m}+ O \left(\frac{1}{m ^2}\right),\]

  donc \(e^{X/m}e^{Y/M} \in B(\operatorname{id},1)\) pour un \(m\) suffisamment grand. Donc \(\log e^{X/m}e^{Y/m} = \frac{X}{m}+ \frac{Y}{m}+ O \left(\frac{1}{m ^2}\right)\). Alors

  \[\exp(\log e^{X/m}e^{Y/m}) = e^{X/m}e^{Y/m} = \exp \left(\frac{X}{m}+ \frac{Y}{m} + O \left(\frac{1}{m ^2}\right)\right),\]

  donc \[(e^{X/m}e^{Y/m})^{m} = \exp\left(m \left(\frac{X}{m}+ \frac{Y}{m} + O \left(\frac{1}{m ^2}\right)\right)\right) = \exp\left(X + Y + O \left(\frac{1}{m}\right)\right).\]

  Par continuité

  \[\exp(X+Y) = \lim_{m \to \infty}(e^{X/m}e^{Y/m})^{m}.\]
\end{proof}

\begin{prop}
  Pour \(X \in \mathbb{M}(n,\mathbb{C})\), \(\operatorname{det}(e^{X}) = e^{\operatorname{Tr}(X)}\).
\end{prop}

\begin{proof}
  On rappelle que \(C e^{X}C ^{-1} = e^{C X C ^{-1}}\), donc si \(X\) est diagonalisable avec les valeurs propres \(z_j\), alors \(e^{X}\) est diagonalisable avec les valeurs propres \(e^{z_j}\). Donc

  \[\operatorname{Tr}(X) = \sum_{j=1}^{m}z_j, e^{\operatorname{Tr}(A)} = \prod e^{z_j} = \operatorname{det}(e^{X}).\]

  Soit \(X \in \mathbb{M}(n,\mathbb{C})\) et \(A \displaystyle \lim_{m \to \infty} X_m \), avec \(X_m\) diagonalisable, alors \[\operatorname{det}(e^{X}) = \operatorname{det}(e^{\lim X_m}) = \lim \operatorname{det}(e^{X_m}) = \lim e^{\operatorname{Tr}(X_m)} = e^{\operatorname{Tr}(X)}.\]
\end{proof}

\begin{definition}
  \(A(\cdot)\in \mathcal{C}^0(\mathbb{R}, GL(n,\mathbb{C}))\) est appelé sous-groupe à un paramètre de \(GL(n,\mathbb{C})\). Si \(A(0) = \operatorname{id}\) et \(A(t+s) = A(t)A(s), \forall s,t \in \mathbb{R}\).
\end{definition}

\begin{thm}\label{unique-parametre}
  Soit \(A(\cdot)\) un sous-groupe à un paramètre de \(GL(n,\mathbb{C})\), alors il existe un unique \(X \in \mathbb{M}(n,\mathbb{C})\) tel que

  \[A(t)=e^{tX}.\]

  Donc \(A \in \mathcal{C}^1(\mathbb{R}, GL(n,\mathbb{C}))\) et \(X = \frac{d}{dt}A(t)\bigm|_{t=0}\).
\end{thm}

Pour la preuve on utilise

\begin{lemma}
  Soit \(\varepsilon \less \frac{1}{2}\log(z)\) et \(B \in V = \exp(B(0,\varepsilon)) = \{ e^{D}, D \in \mathbb{M}(n,\mathbb{C}), \left\Vert D \right\Vert \less \varepsilon \}\), alors \[\sqrt{B}:= e^{\frac{1}{2}\log(B)} \in V\]
   est l'unique racine carrée dans \(V\) telle que \(\sqrt{B}\sqrt{B} = B\).
\end{lemma}

\begin{proof}
  On avait démontré qu'il y avait une correspondance entre \(B(0,\log(z))\) et \(\exp(B(0,\log(z)))\subset B(\operatorname{id},1)\) grâce aux fonctions \(\log\) et \(\exp\). C'est un homéomorphisme.

  De plus, \(\log(B)\) et \(\frac{1}{2}\log(B)\in B(0,\varepsilon)\) et \(\sqrt{B}\sqrt{B} = e^{\log(B)}  =B\).

  Soit \(C \in V\) tel que \(C ^2 = B\), alors \(e^{\log(C)} = C\) et \(e^{2 \log( C)} = C ^2 = B = e^{\log(B)}\). Comme \(\left\Vert 2 \log(C) \right\Vert \less \log(2)\), on a \(2 \log(C) = \log(B)\) et \(C = \sqrt{B}\).
\end{proof}

\begin{proof}[Démonstration du théorème \ref{unique-parametre}]
  Si \(X\) existe, alors il est unique, car \(X = \frac{d}{dt}A(t)\bigm|_{t=0}\). Le \(V\) du lemme est ouvert, donc \(A(t)\in V\) pour \(\left\lvert t \right\rvert \leq t_0\) pour \(t_0 \bg 0\).

  Soit \(X := \frac{1}{t_0} \log(A(t_0))\). Alors \(X \in B(0,\varepsilon)\) et \(A(t_0)=e^{t_0 X}\). Par ailleurs, \(A(\frac{t_0}{2})\in V\) et \((A(t_0/2))^2\), Par unicité de la racine,

  \[A(t_0/2) = e^{t_0/2 X}.\]

  Par récurrence,

  \[A \left(\frac{t_0}{2 ^{k}}\right) = e^{\frac{t_0}{2 ^{k}}X}, k \in \mathbb{N}.\]

  C'est vrai aussi pour \(m \in \mathbb{Z}\). \(\left\{ \frac{m t_0}{2} \right\}_{m \in \mathbb{Z}, k \in \mathbb{N}}\) est dense dans \(\mathbb{R}\), donc par continuité \(A(t)=e^{tA}, \forall t \in \mathbb{R}\).
\end{proof}

\begin{prop}
  \(\exp \in \mathcal{C}^{\infty}(\mathbb{M}(n,\mathbb{C}), \mathbb{M}(n,\mathbb{C}))\).
\end{prop}

\begin{proof}
  \((X ^{m})_{jk}\) est polynomiale. \(\exp X\) est une série absolument convergente de rayon \(\infty\), donc toutes les dérivées partielles existent et sont continues.
\end{proof}

\section{Algèbre de Lie, exemples}

\begin{definition}
  Une \textbf{algèbre de Lie} (de dimension finie) sur \(\mathbb{K}\) est un espace vectoriel \(\mathfrak{g}\) (sur \(\mathbb{K}\) de dimension finie) muni d'un \textbf{crochet de Lie} \([\cdot,\cdot] : \mathfrak{g} \times \mathfrak{g} \longrightarrow \mathfrak{g}\) tel que

  \begin{enumerate}
    \item \([\cdot,\cdot]\) est bilinéaire ;
    \item \([\cdot,\cdot]\) est antisymétrique ;
    \item \textbf{L'identité de Jacobi} est satisfaite : on a, pour tout \(X,Y, Z \in \mathfrak{g}, \lambda \in \mathbb{K}\),

    \begin{enumerate}
      \item \([X,\lambda Y + Z] = \lambda [X,Y]+ [X,Z]\) ;
      \item \([X,Y] = - [Y,X]\) ;
      \item \([X,[Y,Z]] + [Z,[X,Y]]+ [Y,[Z,X]] = 0\).
    \end{enumerate}
  \end{enumerate}

  Si \([X,Y] = 0\), on dit que \(X\) et \(Y\) commutent. Si tous les éléments commutent, on dit que \(\mathfrak{g}\) est abélienne.
\end{definition}
...
\begin{exemple}
  ...
  \begin{enumerate}
    \item Soit \(\mathbb{V}\) un espace vectoriel, \(\operatorname{dim}(\mathbb{V}) \less \infty\) et \(B(\mathbb{V})\) l'ensemble des applications linéaires, alors \(B(\mathbb{V})\) est une algèbre de Lie, avec

    \[[X,Y] = XY - YX.\]

    \item \(\mathfrak{g} \subset B(\mathbb{V})\) un sous-espace tel que \([\mathfrak{g}, \mathfrak{g}]\subset \mathfrak{g}\).

    \item \(\mathfrak{g}(\mathbb{V}):=\{ X \in B(\mathbb{V}), \operatorname{Tr}(X)=0 \}\). En effet,

    \[\operatorname{Tr}(XY-YX) = \operatorname{Tr}(XY)-\operatorname{Tr}(YX) = 0.\]

    \item \(\mathfrak{g}:= \mathbb{R}^3, [X,Y]=x \wedge y\). En effet,

    \begin{enumerate}
      \item bilinéaire ;
      \item antisymétrique par construction \(x \wedge y = - y \wedge x\) ;
      \item \(x \wedge (y\wedge z)+ z\wedge (x \wedge y)+y\wedge(z \wedge x)\), car pour la base canonique, on a

      \begin{enumerate}
        \item pour \(i,j,k\) différents,

        \[e_i \wedge (e_j \wedge e_k) + e_k \wedge (e_i \wedge e_j)+ e_j \wedge (e_k \wedge e_i) = 0.\]
        \item \(i \neq j = k\) : \(e_i \wedge (e_j \wedge e_k) + e_k \wedge (e_i \wedge e_j)+ e_j \wedge (e_k \wedge e_i) = 0.\)
        \item Si tous les indices sont les mêmes, les trois termes s'annullent.
      \end{enumerate}
...
\begin{definition}\marginnote{22-11-2023}
  Soit \((\mathfrak{g}, [\cdot,\cdot])\) une algèbre de Lie et \(\mathfrak{h}\subset \mathfrak{g}\) un sous-espace tel que \([\mathfrak{h}, \mathfrak{h}]\subset\mathfrak{h}\) est

  \begin{enumerate}
    \item appelé \textbf{sous-algèbre de Lie} de \(\mathfrak{g}\) et
    \item \textbf{idéal} si \([X,Y] \in \mathfrak{h}\)
    \item Le centre de \(\mathfrak{g}\) est l'idéal \(\{ X \in \mathfrak{g}, [Y,X] = 0, \forall Y \in \mathfrak{g}\}\).
  \end{enumerate}
\end{definition}

\begin{definition}
  Soient \(\mathfrak{g}, \mathfrak{h}\) des algèbres de Lie de dimension finie. \(\varphi\) est appelé \textbf{homomorphisme d'algèbres de Lie} si \(\varphi([X,Y]) = [\varphi(X), \varphi(Y)], \forall X,Y \in \mathfrak{g}\).
  Si \(\varphi\) est une bijection, on l'appelle \textbf{isomorphisme d'algèbre de Lie} et \textbf{automorphisme d'algèbre de Lie} si \(\mathfrak{g} = \mathfrak{h}\).
\end{definition}

\begin{prop}
  \(\operatorname{Ker}(\varphi)\) est un idéal.
\end{prop}

\begin{exemple}
  \(\mathfrak{g} = \mathbb{R}^3\) avec le produit vectoriel et \(\mathfrak{h} = \{ X \in \mathbb{M}(3,\mathbb{R}) \}\) avec le commutateur. On pose \(\varphi : \mathfrak{g} \longrightarrow \mathfrak{h}\),

  \[\varphi(x_1,x_2,x_3) = \begin{pmatrix}
  0 & -x_3 & x_2 \\
  x_3 & 0 & -x_1 \\
  -x_2 & x_1 & 0
  \end{pmatrix}\]
...
\begin{exemple}
  \(\mathbb{K} = \{ X \in \mathbb{M}(2,\mathbb{C}), \operatorname{Tr}(X) = 0 \}\) avec commutateur. \(\psi : \mathfrak{h} \longrightarrow \mathbb{K}\) défini comme suit :

  \[\begin{pmatrix}
  0 & -c & b\\
  c & 0 & -a \\
  b & a & 0
  \end{pmatrix} \longmapsto \frac{1}{2 i}\begin{pmatrix}
  c & a - ib \\
  a + ib & -c
  \end{pmatrix}\]
...
\begin{definition}
  Pour \(x \in \mathfrak{g}\) une algèbre de Lie, soit \(\operatorname{ad}_X : \mathfrak{g} \longrightarrow \mathfrak{g}, \operatorname{ad}_X := [X,Y]\).
  \(\operatorname{ad} : \mathfrak{g} \longrightarrow \mathbb{B}(\mathfrak{g}), X \mapsto \operatorname{ad}_X\) est appelé l'application adjointe (ou représentation adjointe).
\end{definition}

\begin{prop}
  \(\operatorname{ad} : \mathfrak{g} \longrightarrow \mathbb{B}(\mathfrak{g})\) est un homomorphisme d'algèbres de Lie, i. e.

  \[\operatorname{ad}_{[X,Z]} = [\operatorname{ad}_X,\operatorname{ad}_Z].\]
\end{prop}

\begin{thm}
  Toute algèbre de Lie (de dimension finie) est isomorphe à une sous-algèbre de Lie de \(\mathbb{M}((n,\mathbb{K}))\).
\end{thm}

\begin{proof}
  Cas où \(\mathfrak{g}\) n'admet pas d'idéal non trivial, alors \(\operatorname{Ker}(\operatorname{ad}) = \text{ centre de } \mathfrak{g}\) est trivial, \(\mathfrak{g}\) est isomorphe à \(\operatorname{Im}(\operatorname{ad}) = \mathbb{B}(\mathfrak{g})\).

  Dans le cas général, c'est admis.
\end{proof}

\begin{definition}
  La \textbf{somme directe de deux algèbres de Lie} \(\mathfrak{g}_1,\mathfrak{g}_2\) est l'algèbre de Lie
  \begin{gather*}
    \mathfrak{g}_1 \oplus \mathfrak{g}_2 = \{ (x_1,x_2)\in \mathfrak{g}_1 \times \mathfrak{g}_2, x_j \in \mathfrak{g}_j \} \\
    [(x_1,x_2),(y_1,y_2)]:= ([x_1,y_1],[x_2,y_2]).
  \end{gather*}
\end{definition}

\begin{definition}
  Une algèbre de Lie \(\mathfrak{g}\) est appelée :

  \begin{enumerate}
    \item \textbf{irréductible} si elle n'admet pas d'idéal différent de \(\{ 0 \}\) et \(\mathfrak{g}\) ;
    \item \textbf{simple} si elle est irréductible et \(\operatorname{dim}(\mathfrak{g}) \bg 1\) (si et seulement si elle est irréductible et non abélienne) ;
    \item \textbf{semi-simple} si

    \[g = \bigoplus _{i=1}^{n} \mathfrak{g}_i,\]

    avec \(\mathfrak{g}_i \subset \mathfrak{g}\) des sous-algèbres de Lie.
  \end{enumerate}

\end{definition}

\begin{exemple}
  \(\mathfrak{S}\mathfrak{l}(2,\mathbb{C}) = \{ X \in \mathbb{M}(2,\mathbb{C}), \operatorname{Tr}(X)=0\}\) est simple.
\end{exemple}

\begin{proof}
  \(X \in \mathfrak{S}\mathfrak{l} \iff X = \displaystyle \begin{pmatrix}
  b & a \\
  c & -b
  \end{pmatrix},a,b,c \in \mathbb{C}\).

  Donc \(\mathfrak{S}\mathfrak{l}\) est un espace vectoriel sur \(\mathbb{C}\) engendré par \(H,X,Y\) avec
...
\end{pmatrix},X = \begin{pmatrix}
  ...
  On a \([X,Y] = H, [H,X] = 2 X, [H,Y] = - 2 Y\).

  Soit \(\mathfrak{h}\) un idéal non nul, \(Z = a X + b H + c Y \in \mathfrak{h}\). On a, si \(c \neq 0\),

  \[[X,[X,Z]] = [X,- 2 b X + c H] = - 2 c X.\]
  On a \([X,Y] = H, [H,Y] = - 2 Y\), donc \(X \in \mathfrak{h}, H \in \mathfrak{h}, Y \in \mathfrak{h}\)
\end{proof}

\subsection{Algèbre de Lie et groupe de Lie matriciel}

\begin{definition}
  Soit \(G\) un groupe de Lie matriciel. Son algèbre de Lie est

  \[\mathfrak{g} = \{ X \in \mathbb{M}(n,\mathbb{C}), e^{tX}\in G , \forall t \in \mathbb{R} \}.\]

  On a \(\mid\exp(ity)\mid = 1, \forall t \).
\end{definition}

\begin{prop}
  De manière générale, on verra que \(G\) est une sous-variété différentielle de \(GL(n,\mathbb{C})\) et \(\mathfrak{g}\) est un sous-espace tangent à l'identité.
\end{prop}

\begin{prop}
  \(X \in \mathfrak{g}\) si et seulement si \(e^{X} \in G_0\), la composante connexe de l'identité.
\end{prop}

\begin{proof}
  \(c(t) = e^{tX}, t \in [0,1]\) est continue et \(c(0) = \operatorname{id}\).
\end{proof}

\begin{prop}
  Soit \(G\) un groupe de Lie matriciel et \(\mathfrak{g}\) son algèbre de Lie. Alors pour \(X,Y \in \mathfrak{g}\),

  \begin{enumerate}
    \item \(A X A ^{-1} \in \mathfrak{g}, \forall A \in G\).
    \item \(X + sY \in \mathfrak{g}, \forall s \in \mathbb{R}\).
    \item \(XY - Y X \in \mathfrak{g}\).
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item On a \(A e^{tX}A ^{-1} = e^{tA x A ^{-1}}\) (série).
    \item \(e^{tX}\in G, \forall t\), alors \(e^{st X}, \forall s \in \mathbb{R},t \in \mathbb{R}\). Par la formule du produit de Lie,

    \[e^{t(X+Y)} = \lim_{m \to \infty}(e^{X/m}e^{Y/m})^{m} \in G,\]

    car dans \(GL(n,\mathbb{C})\) et \(G\) est fermé.
    \item\marginnote{23-11-2023}

    \[\frac{d}{dt}\left(e^{tX}Ye^{-tX}\right) = \frac{d}{dt}\left(e^{tX}\right)Y e^{-tX}+ e^{tX}Y \frac{d}{dt}e^{-tX},\]

    donc, en prenant \(t=0\),

    \[[X,Y] = \lim_{t \to 0}\frac{e^{tX}Y e^{-tX}(-Y)}{t},\]

    \(\mathfrak{g}\) est fermé donc \([X,Y]\in \mathfrak{g}\).
  \end{enumerate}
\end{proof}

\begin{corollary}
  L'algèbre de Lie d'un groupe de Lie matriciel est donc une sous-algèbre réelle de \(\mathbb{M}(n,\mathbb{C})\) avec \([X,Y] = XY-YX\).
\end{corollary}

\begin{corollary}
  Si un groupe de Lie matriciel de \(G\) est abélien, alors son algèbre de Lie est abélienne.
\end{corollary}

\paragraph{Tableau récapitulant les principaux groupes de matrices et leurs algèbres associées}
...
\begin{tabular}{|c|c|c|}
  \hline
  Groupe de Lie & Algèbre de Lie & Dimension\\
  \hline
  \(GL(n,\mathbb{K})\) & \(\mathfrak{gL}(n,\mathbb{K}) = \mathbb{M}(n,\mathbb{K})\) & \(2 n ^2\) si \(\mathbb{K} = \mathbb{C}\), \(n ^2\) si \(\mathbb{K} = \mathbb{R}\) \\
  \hline
  \(SL(n,\mathbb{K})\) & \(\mathfrak{sL}(n,\mathbb{K}) = \{ X \in \mathbb{M}(n,\mathbb{K}), \operatorname{Tr}(X) = 0 \}\) & \(2(n ^2- 1)\) si \(\mathbb{C}\), \(2 n ^2 - 1\) si \(\mathbb{R}\)\\
  \hline
  \(U(n)\) & \(\mathfrak{U}(n) = \{ X ; X ^{*} = - X \}\) & \(n ^2\) \\
  \hline
  \(SU(n)\) & \(\mathfrak{SU}(n) = \{ X ; X ^{*} = - X \text{ et } \operatorname{Tr}(X) = 0\}\) & \(n ^2 - 1\)\\
  \hline
  \(O(n)\) & \(\mathfrak{O}(n) = \{ X \in \mathbb{M}(n,\mathbb{R}), X ^{t} = -X \}\) & \(\displaystyle\frac{n ^2 - 1}{2}\) \\
  \hline
  \(SO(n)\) & \(\{ X \in \mathbb{M}(n,\mathbb{R}), X ^{t} = -X \text{ et } \operatorname{Tr}(X) = 0  \}\) & \(\displaystyle \frac{n ^2 - 1}{2}\) \\
  \hline
  \(O(p,q)\) & \(\mathfrak{O}(p,q) = \left\{ X \in \mathbb{M}(n,\mathbb{R}), g X ^{t}g = X, g =\begin{pmatrix}
  \operatorname{id}_p & 0\\
  0 & -\operatorname{id}_q
  \end{pmatrix} \right\}\) & \ \\
  \hline
  \(Sp(2n)\) & \(\mathfrak{Sp}(2n) = \left\{ X \in \mathbb{M}(n,\mathbb{R}), gX ^{t}g =  \begin{pmatrix}
  0 & -\operatorname{id}_n \\
  \operatorname{id}_n & 0
  \end{pmatrix}\right\}\) & \(n(2n+1)\)\\
  \hline
\end{tabular}

\section{Homomorphismes d'algèbres de Lie}

\begin{prop}
  Soient \(G,H\) des groupes de Lie matriciels et \(\mathfrak{g},\mathfrak{h}\) les algèbres de Lie associées. Soit \(\varphi  : G \longrightarrow H\) un homomorphisme. Alors il existe un unique \(\widehat{\varphi} \in \mathbb{B}(\mathfrak{g},\mathfrak{h})\) tel que

  \[\varphi(e^{X}) = e^{\widehat{\varphi}(X)}, \forall X \in \mathfrak{g}.\]

  \(\widehat{\varphi}\) est un homomorphisme d'algèbres de Lie

  \[\forall X,Y \in \mathfrak{g}, \widehat{\varphi}([X,Y]) = [\widehat{\varphi}(X), \widehat{\varphi}(Y)]\]

  et on a

  \begin{enumerate}
    \item \(\widehat{\varphi}(A X A ^{-1}) = \varphi(A)\widehat{\varphi}(X)\varphi(A ^{-1}), \forall X \in \mathfrak{g}, A \in G\) ;
    \item \(\widehat{\varphi}(X) = \displaystyle \frac{d}{dt}\varphi(e^{tX})\bigm|_{t=0}\).
  \end{enumerate}
\end{prop}

\begin{proof}
  \(t \in \mathbb{R} \longmapsto \varphi(e^{iX})\) est un groupe à un paramètre, car \(\varphi\) est continu. On avait démontré que \(\varphi(e^{tX}) = e^{tZ}\) pour une matrice \(Z\) unique.

  Soit \(\widehat{\varphi}(X):= Z\). On a alors \(\varphi(e^{X}) = e^{\widehat{\varphi}(X)}\) et \(\widehat{\varphi}(sX) = s \widehat{\varphi}(X)\). On a

  \begin{gather*}
    e^{t \widehat{\varphi}(X+Y)} = \varphi(e^{t(X+Y)}) = \varphi \left(\lim_{m \to \infty} (e^{tX/m}e^{tY/m})^{m} \right) = \lim_{m \to \infty}\varphi(e^{tX/m})\varphi(e^{tY/m}) )\\
     = \lim_{m\to \infty} e^{t/m \widehat{\varphi}(X)}e^{t/m \widehat{\varphi}(Y)} = e^{t(\widehat{\varphi}(X) \widehat{\varphi}(Y))},
  \end{gather*}

  donc

  \begin{gather*}
    \frac{d}{dt}e^{t \widehat{\varphi}(X+Y)}\bigm|_{t=0} = \widehat{\varphi}(X+Y) = \frac{d}{dt} e^{t \widehat{\varphi}(X) + \widehat{\varphi}(Y)} = \widehat{\varphi}(X)+ \widehat{\varphi}(Y),
  \end{gather*}

  donc \(\widehat{\varphi}\) est linéaire.
\end{proof}

\begin{prop}
  Soit \(\varphi : G \longrightarrow H\) homomorphisme de groupes de Lie matriciels et \(\widehat{\varphi} : \mathfrak{g} \longrightarrow \mathfrak{h}\) l'homomorphisme d'algèbres de Lie associé, alors

  \begin{enumerate}
    \item \(\operatorname{Ker}(\varphi)\) est un sous-groupe distingué fermé.
    \item Son algèbre de Lie est \(\operatorname{Ker}(\varphi)\).
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \[\varphi(U X U ^{-1}) = \varphi(U)\varphi(X)\varphi(U ^{-1}) = \operatorname{id},\]

  \(\operatorname{Ker}(\varphi) = \varphi ^{-1}(\{ \operatorname{id} \})\) est fermé car \(\varphi\) continue. Alors

  \[\widehat{\varphi}(X) = 0 \implies e^{t \widehat{\varphi}(X)} = \operatorname{id} \implies e^{tX} \in \operatorname{Ker}(\varphi) \implies X \in \operatorname{Lie}(\operatorname{Ker}(\varphi))\]

  et

  \[e^{tX} \in \operatorname{Ker}(\varphi) \forall t \implies 1 = \varphi(e^{tX}) = e^{t \widehat{\varphi}(X)} \forall t \implies \widehat{\varphi}(X)=0.\]
\end{proof}

\begin{definition}
  Soit \(G\) un groupe de Lie matriciel et \(\mathfrak{g}\) son algèbre de Lie. Pour \(A \in G\), l'application

  \[\operatorname{Ad}_{A} : \mathfrak{g} \longrightarrow \mathfrak{g}, \quad \operatorname{Ad}_A(X) = A X A ^{-1}\]

  est appelée l'\textbf{application adjointe} (alias représentation adjointe).
\end{definition}

\begin{remark}
  \(A e^{tX}A ^{-1} = e^{tA X A ^{-1}}\), donc \(\operatorname{Ad}\) est bien définie.
\end{remark}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(\operatorname{Ad} : G \longrightarrow GL(\mathfrak{g})\) est un homomorphisme.
    \item \(\operatorname{Im}(\operatorname{Ad}_A)\) est une sous-algèbre de Lie de \(GL(\mathfrak{g})\) :

    \[\forall X,Y \in \mathfrak{g}, \operatorname{Ad}_{A}([X,Y]) = [\operatorname{Ad}_A(X), \operatorname{Ad}_A(Y)].\]
    \item Son application associée est \(\operatorname{ad} = \widehat{\operatorname{Ad}}\), avec

    \[\operatorname{ad} : \mathfrak{g} \longrightarrow \mathfrak{gl}(\mathfrak{g}), \operatorname{ad}_X(Y) = [X,Y].\]
  \end{enumerate}
\end{prop}

\begin{proof}
  On a \(\operatorname{Ad}_A \operatorname{Ad}_B = \operatorname{Ad}_{AB}, (\operatorname{Ad}_A)^{-1} = \operatorname{Ad}_{A ^{-1}}\). De plus, \(\operatorname{Ad} \in \mathcal{C}^0(G,GL(\mathfrak{g}))\), car les opérations linéaires de groupes sont continues.

  \[\operatorname{Ad}_A([X,Y]) = A X A ^{-1} A Y A ^{-1} - A Y A ^{-1} A X A ^{-1} = [\operatorname{Ad}_A(X), \operatorname{Ad}_A(Y)].\]

  On avait démontré que \(\widehat{\operatorname{Ad}}(X) = \displaystyle \frac{d}{dt}\operatorname{Ad} _{e^{tX}}\bigm|_{t=0}\),

  \[\frac{d}{dt}\operatorname{Ad} _{e^{tX}}(Y)\bigm|_{t=0} = \frac{d}{dt}e^{tX}Y e^{-tX}\bigm|_{0} = [X,Y] = \operatorname{ad}_X(Y).\]
\end{proof}

\begin{corollary}
  Pour tout \(X,Y \in \mathbb{M}(n,\mathbb{C})\), on a

  \[e^{X}Ye^{-X} = e^{\operatorname{ad}(X)}(Y)= \sum_{0}^{\infty} \frac{1}{m!}\operatorname{ad}_X ^{m}(Y). \]
\end{corollary}

\begin{definition}\marginnote{29-11-2023}
  Soit une algèbre de Lie \(\mathfrak{g} \in \mathbb{M}(n,\mathbb{C})\) telle que \(iX \in \mathfrak{g}, \forall X \neq 0 \in \mathfrak{g}\), alors la complexification

  \[\mathfrak{g} _{\mathbb{C}} \{  X+ iY \in \mathbb{M}(n,\mathbb{C}) ; X, Y \in \mathfrak{g} \},\]

  avec \([X_1 + i X_2, Y_1 + i Y_2] = [X_1, Y_1]- [X_2,Y_2]+ i([X_2,Y_1]+[X_1,Y_2])\).
\end{definition}

\begin{prop}
  Soit \(\mathfrak{g}\) une algèbre de Lie réelle et \(\mathfrak{h}\) une algèbre de Lie complexe et \(\pi : \mathfrak{g} \longrightarrow \mathfrak{h}\) un homomorphisme d'algèbre de Lie réelle. Alors il existe un unique homomorphisme d'algèbre de Lie complexe \(\pi _{\mathbb{C}} : \mathfrak{g} _{\mathbb{C}} \longrightarrow \mathfrak{h}\) qui prolonge \(\pi\).
\end{prop}

\begin{proof}
  \(\pi _{\mathbb{C}}(X+ iY) := \pi(X)+ i \pi(Y)\).
\end{proof}
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(\mathfrak{gL}(n,\mathbb{R}) _{\mathbb{C}} \simeq \mathfrak{gl}(n,\mathbb{C})\) ;
    \item \(\mathfrak{U}(n)_{\mathbb{C}} \simeq \mathfrak{gL}(n,\mathbb{C})\) ;
    \item \(\mathfrak{SU}(n)_{\mathbb{C}} \simeq \mathfrak{sL}(n,\mathbb{C})\) ;
    \item \(\mathfrak{SL}(n,\mathbb{R}) _{\mathbb{C}} \simeq \mathfrak{sL}(n,\mathbb{C}).\)
  \end{enumerate}
\end{prop}

\begin{prop}
  \(\operatorname{dim} \mathfrak{sL}(n,\mathbb{R}) = 2 n ^2 - 1 \neq n ^2 - 1 = \operatorname{dim} \mathfrak{su}(n)\), donc \(\mathfrak{sL}(n,\mathbb{R})\) n'est pas équivalent à \(\mathfrak{su}(n)\).
\end{prop}

\subsection{L'application \(\exp : \mathfrak{g} \longrightarrow G\)}

Nous démontrons que la restriction de cette fonction sur un voisinage de 0 est bijective sur son image.
...
\begin{prop}
  ...
  \begin{enumerate}
    \item \(\exp : \mathfrak{gL}(2,\mathbb{C}) \longrightarrow SL(2,\mathbb{C})\) n'este pas surjectif.
    \item \(\exp : \mathfrak{su}(2) \longrightarrow SU(2)\) n'est pas injectif.
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    -1 & 1 \\
    0 & -1
    \end{pmatrix}\) n'est pas diagonalisable, donc \(e^{X} = \begin{pmatrix}
    -1 & 1 \\
    0 & -1
    \end{pmatrix} \implies X\) n'est pas diagonalisable, \(0 \neq \lambda\) (valeur propre de \(X\)), alors \(\operatorname{Tr}(X) =0\) implique que \(-\lambda\) est valeur propre de \(X\), donc diagonalisable.

    \item \(\exp(0) = \exp \begin{pmatrix}
    2 \pi i & 0 \\
    0 & - 2 \pi i
...
\begin{thm}
  ...
  \begin{enumerate}
    \item Soit \(G\) un groupe de Lie matriciel et \(\mathfrak{g}\) son algèbre de Lie. Alors il existe \(\varepsilon \less \log(2)\) tel que \(A = e^{X}\) pour \(X \in \mathbb{M}(n,\mathbb{C})\), \(\left\Vert X \right\Vert \less \varepsilon\) et \(A \in G \iff \log(A) \in \mathfrak{g}\).
    \item \(B(0,\varepsilon) \cap \mathfrak{g} \stackrel{\exp}{\longrightarrow} \exp(B(0,\varepsilon))\cap G\) est un homéomorphisme.
  \end{enumerate}
\end{thm}
...
\begin{proof}
  ...
  \begin{lemma}
    Soit \((B_m)_{m \in \mathbb{N}}\subset B(\operatorname{id},1)\cap G \setminus \{ \operatorname{id} \}\), \(\displaystyle \lim_{m \to +\infty} B_m = \operatorname{id}\).

    On a : si \(\displaystyle\lim_{m \to +\infty}\displaystyle \frac{Y_m}{\left\Vert Y_m \right\Vert } = Y \in \mathbb{M}(n,\mathbb{C})\) existe, alors \(Y \in \mathfrak{g}\).
  \end{lemma}
\end{proof}

\begin{corollary}
  Un groupe de Lie matriciel est une sous-variété différentielle de \(GL(n,\mathbb{C})\) de dimension de \(\mathfrak{g}\).
\end{corollary}

\begin{corollary}
  Soit \(G\) un groupe de Lie matriciel. Alors \(\operatorname{id} \in \mathfrak{g}\), son algèbre de Lie si et seulement si \(\gamma \in \mathcal{C}^{\infty}((-a,a), \mathbb{M}(n,\mathbb{C})), \gamma(0) = \operatorname{id}, \dot{\gamma}(0) = X\), donc \(\mathfrak{g} = T _{\operatorname{id}}G\).
\end{corollary}

\begin{corollary}
  Un homomorphisme continu entre deux groupe de Lie matriciels est \(\mathcal{C}^{\infty}\).
\end{corollary}

\begin{corollary}
  Soit \(G\) un groupe de Lie connexe, alors pour \(A \in G\), il existe \(X_1, \dots, X_m \in \mathfrak{g}\) tel que \(A = e^{X_1}\dots e^{X_m}\).
\end{corollary}

\begin{remark}
  \(SL(2,\mathbb{C})\) est connexe, mais \(\begin{pmatrix}
  -1 & 1 \\
  0 & -1
  \end{pmatrix} \neq e^{X}, \forall X \in \mathfrak{sl}(2,\mathbb{C})\).
\end{remark}

\begin{lemma}
  \(A \in \mathcal{C}^0([a,b],GL(n,\mathbb{C}))\), alors pour tout \(\varepsilon \bg 0\), il existe \(\delta\), \(\left\lvert s-t \right\rvert \less \delta \implies \left\Vert A(s)A(t)^{-1} - \operatorname{id} \right\Vert  \less \varepsilon\).
\end{lemma}

\begin{corollary}
  Soient \(G,H\) des groupes de Lie matriciel, \(\mathfrak{g},\mathfrak{h}\) leurs algèbres de Lie. Soit \(\varphi_1,\varphi_2 : G \longrightarrow H\) deux Homomorphismes de \(G\) de Lie et \(\widehat{\varphi_1},\widehat{\varphi_2} : \mathfrak{g} \longrightarrow \mathfrak{h}\) les homomorphismes associés. Alors

  \[G \text{ connexe } \iff (\widehat{\varphi_1} = \widehat{\varphi_2} \implies \varphi_1 = \varphi_2). \]
\end{corollary}

\section{Représentation de groupe, algèbre \(SU(2) \iff \mathfrak{su}(3), SO(3) \iff \mathfrak{so}(3)\)}\marginnote{01-12-2023}
\begin{prop}
  Soit \(\pi\) une représentation irréductible de \(\mathfrak{su}(2)\), alors il existe une représentation \(\varphi\) irréductible de \(SU(2)\) telle que \(\pi = \widehat{\varphi}\).
\end{prop}

\begin{proof}
  En effet, si \(\pi_C\) est irréductible et isomorphe à \(\pi _{m \mathbb{C}}\), et \(\pi _{m} = \widehat{\prod_{m}}\), donc \(\pi = \hat{\varphi}\), avec \(\varphi\) isomorphe à \(\prod_m\).
\end{proof}

\begin{remark}
  \(f : SU(2) \longrightarrow SO(\mathfrak{su}(2))\), \(f(U)(X):= UX U ^{*} = \operatorname{Ad}_U (X)\) est surjectif et \(\operatorname{Ker}(f) = \{ \pm \operatorname{id} \}\).
\end{remark}
