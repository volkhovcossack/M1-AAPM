...
\section{Espaces localement convexes métrisables, espaces de Fréchet} \marginnote{26-09-2023}
...
\section{Topologie forte et topologie faible sur les espaces de Banach}\marginnote{10-10-2023}
...
On cherche aussi les fonctions test dans \(\mathcal{K}(\Omega)\) ou \(\mathcal{C}^{1}_{0}\) qui ne sont ni des espaces de Banach, ni des espaces de Fréchet. On choisit généralement \(\mathcal{C}^{0}_{0}(\Omega)\).
...
\begin{prop}[Axiomes]
  ...
  \begin{enumerate}
    ...
    \item \emph{Théorème de Beppo-Levi :} si \((f_n)\) est une suite croissante de fonctions mesurables, alors
...
\begin{thm}[{\fontencoding{U}\fontfamily{futs}\selectfont\char 66\relax} De convergence dominée]
...
L'espace \(L ^2\) est engendrée par une base orthonormée (\emph{base hilbertienne}), i.e. il existe une suite \((e_j)_j\) telle que, pour tout \(f \in L ^2\), on a
